---
title: "Konstruktor System::Drawing::Bitmap::Bitmap"
linktitle: "Bitmap"
second_title: "Aspose.Page untuk C++"
description: "Konstruktor System::Drawing::Bitmap::Bitmap. Membuat objek Bitmap baru dari gambar yang ada yang ditentukan dalam C++."
type: docs
weight: 100
url: /id/cpp/system.drawing/bitmap/bitmap/
---
## Bitmap::Bitmap(const SharedPtr\<Image\>\&) constructor


Membuat objek [Bitmap](../) baru dari gambar yang ada yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const SharedPtr<Image> &original)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| asli | const SharedPtr\<Image\>\& | Gambar yang ada untuk membuat gambar bitmap |

## Lihat Juga

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(const SharedPtr\<Image\>\&, const Size\&) constructor


Membuat objek [Bitmap](../) baru dari gambar yang ada yang ditentukan, diubah skalanya ke ukuran yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const SharedPtr<Image> &original, const Size &size)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| asli | const SharedPtr\<Image\>\& | Gambar yang ada untuk membuat gambar bitmap |
| size | const Size\& | Ukuran gambar baru |

## Lihat Juga

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Size](../../size/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(const SharedPtr\<Image\>\&, int, int) constructor


Membuat objek [Bitmap](../) baru dari gambar yang ada yang ditentukan dengan lebar dan tinggi yang diubah skalanya ke nilai yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const SharedPtr<Image> &original, int width, int height)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| asli | const SharedPtr\<Image\>\& | Gambar yang ada untuk membuat gambar bitmap |
| width | int | Lebar gambar baru |
| height | int | Tinggi gambar baru |

## Lihat Juga

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(const SharedPtr\<System::IO::Stream\>\&, bool) constructor


Membuat objek [Bitmap](../) baru dari aliran yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const SharedPtr<System::IO::Stream> &stream, bool useIcm=false)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| stream | const SharedPtr\<System::IO::Stream\>\& | Sebuah stream yang berisi data gambar |
| useIcm | bool | IGNORED |

## Lihat Juga

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Stream](../../../system.io/stream/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(const String\&) constructor


Membuat objek [Bitmap](../) baru dari file yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const String &filename)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| filename | const String\& | Nama file yang berisi data gambar |

## Lihat Juga

* Class [String](../../../system/string/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(const String\&, bool) constructor


Membuat objek [Bitmap](../) baru dari file yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(const String &filename, bool useIcm)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| filename | const String\& | Nama file yang berisi data gambar |
| useIcm | bool | IGNORED |

## Lihat Juga

* Class [String](../../../system/string/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
## Bitmap::Bitmap(int, int, Imaging::PixelFormat) constructor


Membuat objek [Bitmap](../) baru yang mewakili gambar bitmap dengan lebar, tinggi, format piksel, dan data piksel yang ditentukan.

```cpp
System::Drawing::Bitmap::Bitmap(int width, int height, Imaging::PixelFormat format=Imaging::PixelFormat::Format32bppArgb)
```


| Parameter | Tipe | Deskripsi |
| --- | --- | --- |
| width | int | Lebar gambar |
| height | int | Tinggi gambar |
| format | Imaging::PixelFormat | Format piksel gambar |

## Lihat Juga

* Enum [PixelFormat](../../../system.drawing.imaging/pixelformat/)
* Class [Bitmap](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Page for C++](../../../)
