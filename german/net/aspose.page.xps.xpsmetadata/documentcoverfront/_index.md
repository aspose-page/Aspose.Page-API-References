---
title: Class DocumentCoverFront
second_title: Aspose.Page für .NET-API-Referenz
description: Aspose.Page.XPS.XpsMetadata.DocumentCoverFront klas. Beschreibt das vordere AnfangsDeckblatt. Jedes Dokument hat ein separates Blatt. Das Deckblatt sollte aufgedruckt werdenPageMediaSize UndPageMediaType wird für die erste Seite des Dokuments verwendet. Das Deckblatt sollte in Verarbeitungsoptionen integriert werden wie zDocumentDuplex DocumentNUp  wie durch die angegebene Option angegeben. https//docs.microsoft.com/enus/windows/win32/printdocs/documentcoverfront
type: docs
weight: 660
url: /de/net/aspose.page.xps.xpsmetadata/documentcoverfront/
---
## DocumentCoverFront class

Beschreibt das vordere (Anfangs-)Deckblatt. Jedes Dokument hat ein separates Blatt. Das Deckblatt sollte aufgedruckt werden[`PageMediaSize`](../pagemediasize/) Und[`PageMediaType`](../pagemediatype/) wird für die erste Seite des Dokuments verwendet. Das Deckblatt sollte in Verarbeitungsoptionen integriert werden (wie z[`DocumentDuplex`](../documentduplex/) ,[`DocumentNUp`](../documentnup/) ) wie durch die angegebene Option angegeben. https://docs.microsoft.com/en-us/windows/win32/printdocs/documentcoverfront

```csharp
public sealed class DocumentCoverFront : Feature, IDocumentPrintTicketItem, IJobPrintTicketItem
```

## Konstrukteure

| Name | Beschreibung |
| --- | --- |
| [DocumentCoverFront](documentcoverfront/)(params CoverFrontOption[]) | Erstellt eine neue Instanz. |

## Eigenschaften

| Name | Beschreibung |
| --- | --- |
| [Name](../../aspose.page.xps.xpsmetadata/printticketelement/name/) { get; } | Ruft den Elementnamen ab. |

## Methoden

| Name | Beschreibung |
| --- | --- |
| [Add](../../aspose.page.xps.xpsmetadata/feature/add/)(params IFeatureItem[]) | Fügt eine Liste von Elementen am Ende der Elementliste dieser Funktion hinzu. Jeder muss a sein[`Feature`](../feature/) , ein[`Option`](../option/) oder ein[`Property`](../property/) Instanz. |

## Andere Mitglieder

| Name | Beschreibung |
| --- | --- |
| class [CoverFrontOption](documentcoverfront.coverfrontoption/) | Beschreibt die`DocumentCoverFront` Funktionsoptionen. |

### Siehe auch

* class [Feature](../feature/)
* interface [IDocumentPrintTicketItem](../idocumentprintticketitem/)
* interface [IJobPrintTicketItem](../ijobprintticketitem/)
* namensraum [Aspose.Page.XPS.XpsMetadata](../../aspose.page.xps.xpsmetadata/)
* Montage [Aspose.Page](../../)


