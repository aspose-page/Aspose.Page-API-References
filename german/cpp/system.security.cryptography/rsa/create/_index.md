---
title: "System::Security::Cryptography::RSA::Create Methode"
linktitle: "Erstellen"
second_title: "Aspose.Page für C++"
description: "System::Security::Cryptography::RSA::Create Methode. Erstellt die Standard‑RSA‑Algorithmus‑Implementierung in C++."
type: docs
weight: 1500
url: /de/cpp/system.security.cryptography/rsa/create/
---
## RSA::Create() method


Erstellt die Standard‑[RSA](../) Algorithmus‑Implementierung.

```cpp
static SharedPtr<RSA> System::Security::Cryptography::RSA::Create()
```

## Siehe auch

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [RSA](../)
* Class [RSA](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## RSA::Create(const RSAParameters\&) method


Erstellt die Standard‑[RSA](../) Algorithmus‑Implementierung mit angegebenen Parametern.

```cpp
static SharedPtr<RSA> System::Security::Cryptography::RSA::Create(const RSAParameters &parameters)
```


| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| parameters | const RSAParameters\& | Die Parameter für den [RSA](../) Algorithmus. |

## Siehe auch

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [RSA](../)
* Class [RSA](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## RSA::Create(const String\&) method


Erstellt die Standard-Implementierung des [RSA](../)-Algorithmus.

```cpp
static SharedPtr<RSA> System::Security::Cryptography::RSA::Create(const String &alg_name)
```


| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| alg_name | const String\& | Muss "System.Security.Cryptography.RSACryptoServiceProvider" sein. |

## Siehe auch

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [RSA](../)
* Class [String](../../../system/string/)
* Class [RSA](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## RSA::Create(int32_t) method


Erstellt die Standard-Implementierung des [RSA](../)-Algorithmus mit angegebener Schlüssellänge.

```cpp
static SharedPtr<RSA> System::Security::Cryptography::RSA::Create(int32_t key_size_in_bits)
```


| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| key_size_in_bits | int32_t | Die Schlüssellänge in Bit. |

## Siehe auch

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [RSA](../)
* Class [RSA](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
