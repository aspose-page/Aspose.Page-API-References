...
```js
  var fGetPageCount = function (e) {
    ...
    file_reader.onload = (event) => {
      const json = AsposeXPSGetPageCount(event.target.result, e.target.files[0].name);
      if (json.errorCode == 0) {
...
```js
  ...
  const fGetPagesCount = e => {
    const file_reader = new FileReader();
    file_reader.onload = event => {
      AsposePageWebWorker.postMessage({ "operation": 'AsposeXPSGetPageCount', "params": [event.target.result, e.target.files[0].name] }, [event.target.result]);
    };
    file_reader.readAsArrayBuffer(e.target.files[0]);
  };
