---
title: "System::Text::RegularExpressions::Capture class"
linktitle: "Capture"
second_title: "Aspose.Page voor C++"
description: "System::Text::RegularExpressions::Capture class. Resultaat van een enkele subexpressie‑overeenkomst. Objecten van deze klasse mogen alleen worden gealloceerd met de System::MakeObject()‑functie. Maak nooit een instantie van dit type op de stack of met operator new, omdat dit zal leiden tot runtime‑fouten en/of assertiefouten. Wikkel deze klasse altijd in een System::SmartPtr‑pointer en gebruik deze pointer om deze als argument aan functies door te geven in C++."
type: docs
weight: 100
url: /nl/cpp/system.text.regularexpressions/capture/
---
## Capture class


Resultaat van een enkele subexpressie‑overeenkomst. Objecten van deze klasse mogen alleen worden gealloceerd met de [System::MakeObject()](../../system/makeobject/)‑functie. Maak nooit een instantie van dit type op de stack of met operator new, omdat dit zal leiden tot runtime‑fouten en/of assertiefouten. Wikkel deze klasse altijd in een [System::SmartPtr](../../system/smartptr/)‑pointer en gebruik deze pointer om deze als argument aan functies door te geven.

```cpp
class Capture : public System::Object
```

## Methoden

| Methode | Beschrijving |
| --- | --- |
| [Capture](./capture/)(const UStringPtr\&, int, int) | Constructor. |
| [get_Index](./get_index/)() const | Haalt de index op van de vastgelegde substring. |
| [get_Length](./get_length/)() const | Haalt de lengte op van de vastgelegde substring. |
| [get_Value](./get_value/)() const | Haalt de vastgelegde substring op. |
| [ToString](./tostring/)() const override | Haalt de vastgelegde substring op. |
## Zie ook

* Class [Object](../../system/object/)
* Namespace [System::Text::RegularExpressions](../)
* Library [Aspose.Page for C++](../../)
