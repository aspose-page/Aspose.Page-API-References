---
title: "PageMediaType.BackCoating"
second_title: "Справочник API Aspose.Page для Java"
description: "Определяет константы для значения свойства BackCoating."
type: docs
weight: 10
url: /ru/java/com.aspose.xps.metadata/pagemediatype.backcoating/
---
**Inheritance:**
java.lang.Object, [com.aspose.xps.metadata.PrintTicketElement](../../com.aspose.xps.metadata/printticketelement), [com.aspose.xps.metadata.CompositePrintTicketElement](../../com.aspose.xps.metadata/compositeprintticketelement), [com.aspose.xps.metadata.ScoredProperty](../../com.aspose.xps.metadata/scoredproperty)

**All Implemented Interfaces:**
[com.aspose.xps.metadata.PageMediaType.IPageMediaTypeOptionItem](../../com.aspose.xps.metadata/ipagemediatypeoptionitem)
```
public static final class PageMediaType.BackCoating extends ScoredProperty implements PageMediaType.IPageMediaTypeOptionItem
```

Определяет константы для значения свойства  BackCoating .
## Поля

| Поле | Описание |
| --- | --- |
| [Glossy](#Glossy) | Глянцевое значение. |
| [HighGloss](#HighGloss) | Значение HighGloss. |
| [Matte](#Matte) | Значение Matte. |
| [None](#None) | Значение None. |
| [Satin](#Satin) | Значение Satin. |
| [SemiGloss](#SemiGloss) | Значение SemiGloss. |
## Методы

| Метод | Описание |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [getName()](#getName--) | Получает имя элемента. |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### Glossy {#Glossy}
```
public static PageMediaType.BackCoating Glossy
```


Глянцевое значение.

### HighGloss {#HighGloss}
```
public static PageMediaType.BackCoating HighGloss
```


Значение HighGloss.

### Matte {#Matte}
```
public static PageMediaType.BackCoating Matte
```


Значение Matte.

### None {#None}
```
public static PageMediaType.BackCoating None
```


Значение None.

### Satin {#Satin}
```
public static PageMediaType.BackCoating Satin
```


Значение Satin.

### SemiGloss {#SemiGloss}
```
public static PageMediaType.BackCoating SemiGloss
```


Значение SemiGloss.

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getName() {#getName--}
```
public String getName()
```


Получает имя элемента.

**Returns:**
java.lang.String
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| Параметр | Тип | Описание |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

