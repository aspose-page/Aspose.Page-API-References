...
| static [TryEnter](./tryenter/)(const System::SharedPtr\<Object\>\&, int32_t, bool\&) | Пытается в течение указанного времени захватить эксклюзивный блокировочный объект на заданном объекте и атомарно устанавливает значение, указывающее, был ли захват выполнен. |
| static [TryEnter](./tryenter/)(const System::SharedPtr\<Object\>\&, TimeSpan, bool\&) | Пытается в течение указанного времени захватить эксклюзивный блокировочный объект на заданном объекте и атомарно устанавливает значение, указывающее, был ли захват выполнен. |
| static [Wait](./wait/)(const SharedPtr\<Object\>\&, int32_t, bool) | Снимает блокировку с объекта и блокирует текущий поток до повторного захвата блокировки. Если указанный интервал тайм‑аута истекает, поток переходит в очередь готовности. При необходимости выходит из домена синхронизации для синхронизированного контекста перед ожиданием и повторно входит в домен после него. Не реализовано. |
| static [Wait](./wait/)(const SharedPtr\<Object\>\&, TimeSpan, bool) | Снимает блокировку с объекта и блокирует текущий поток до повторного захвата блокировки. Если указанный интервал тайм‑аута истекает, поток переходит в очередь готовности. При необходимости выходит из домена синхронизации для синхронизированного контекста перед ожиданием и повторно входит в домен после него. Не реализовано. |
| static [Wait](./wait/)(const SharedPtr\<Object\>\&, int32_t) | Снимает блокировку с объекта и блокирует текущий поток до повторного захвата блокировки. Если указанный интервал тайм‑аута истекает, поток переходит в очередь готовности. Не реализовано. |
| static [Wait](./wait/)(const SharedPtr\<Object\>\&, TimeSpan) | Снимает блокировку с объекта и блокирует текущий поток до повторного захвата блокировки. Если указанный интервал тайм‑аута истекает, поток переходит в очередь готовности. Не реализовано. |
| static [Wait](./wait/)(const SharedPtr\<Object\>\&) | Снимает блокировку с объекта и блокирует текущий поток до повторного захвата блокировки. Не реализовано. |
## Примечания
