...
description: "System::Threading::ThreadPool class. API пула потоков, позволяющий помещать задачи в очередь для обработки пулом рабочих потоков. Это статический тип без сервисов экземпляра. Вы никогда не должны создавать его экземпляры каким-либо способом в C++."
type: docs
weight: 1300
...
| static [JoinAllThreads](./joinallthreads/)() | Объединяет все принадлежащие потоки. Ожидает бесконечно. |
| [operator=](./operator=/)(const ThreadPool\&) | Копирование запрещено. |
| static [QueueUserWorkItem](./queueuserworkitem/)(WaitCallback) | Помещает рабочий элемент в очередь, который присутствует с обратным вызовом без параметров. |
| static [QueueUserWorkItem](./queueuserworkitem/)(WaitCallback, const System::SharedPtr\<System::Object\>\&) | Помещает рабочий элемент в очередь, который присутствует с обратным вызовом без параметров. |
...
| [ThreadPool](./threadpool/)(const ThreadPool\&) | Копирование запрещено. |
## Примечания
