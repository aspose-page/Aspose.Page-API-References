---
title: "System::TimeZoneInfo::GetAmbiguousTimeOffsets метод"
linktitle: "GetAmbiguousTimeOffsets"
second_title: "Aspose.Page для C++"
description: "System::TimeZoneInfo::GetAmbiguousTimeOffsets метод. Получает даты и времена UTC, к которым может быть сопоставлена указанная дата и время в C++."
type: docs
weight: 2200
url: /ru/cpp/system/timezoneinfo/getambiguoustimeoffsets/
---
## TimeZoneInfo::GetAmbiguousTimeOffsets(const DateTimeOffset\&) const method


Получает даты и время UTC, к которым может быть сопоставлена указанная дата и время.

```cpp
ArrayPtr<TimeSpan> System::TimeZoneInfo::GetAmbiguousTimeOffsets(const DateTimeOffset &date_time_offset) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| date_time_offset | const DateTimeOffset\& | Дата и время. |

### ReturnValue

[Array](../../array/) of UTC dates and times.

## См. также

* Typedef [ArrayPtr](../../arrayptr/)
* Class [TimeSpan](../../timespan/)
* Class [DateTimeOffset](../../datetimeoffset/)
* Class [TimeZoneInfo](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## TimeZoneInfo::GetAmbiguousTimeOffsets(DateTime) const method


Получает даты и время UTC, к которым может быть сопоставлена указанная дата и время.

```cpp
ArrayPtr<TimeSpan> System::TimeZoneInfo::GetAmbiguousTimeOffsets(DateTime date_time) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| date_time | DateTime | Дата и время. |

### ReturnValue

[Array](../../array/) of UTC dates and times.

## См. также

* Typedef [ArrayPtr](../../arrayptr/)
* Class [TimeSpan](../../timespan/)
* Class [DateTime](../../datetime/)
* Class [TimeZoneInfo](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
