---
title: "Метод System::String::operator+="
linktitle: "operator+="
second_title: "Aspose.Page для C++"
description: "Метод System::String::operator+=. Оператор присваивания с конкатенацией в C++."
type: docs
weight: 2900
url: /ru/cpp/system/string/operator+=/
---
## String::operator+=(char_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(char_t c)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| c | char_t | Символ, добавляемый в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(const String\&) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(const String &str)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | [String](../) для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(double) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(double value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | double | [Double](../../double/) для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(int16_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(int16_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | int16_t | Short для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(int32_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(int32_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | int32_t | Int для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(int64_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(int64_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | int64_t | Long для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(T) method


Оператор присваивания конкатенации.

```cpp
template<typename T,std::enable_if_t< std::is_same< T, bool >::value > *> String & System::String::operator+=(T value)
```


| Параметр | Описание |
| --- | --- |
| T | Тип значения для конкатенации со строкой. Должен быть bool |

| Параметр | Тип | Описание |
| --- | --- | --- |
| value | T | [Boolean](../../boolean/) для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(uint16_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(uint16_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | uint16_t | Unsigned short для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(uint32_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(uint32_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | uint32_t | Unsigned int для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(uint64_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(uint64_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | uint64_t | Unsigned long для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## String::operator+=(uint8_t) method


Оператор присваивания конкатенации.

```cpp
String & System::String::operator+=(uint8_t value)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| value | uint8_t | [Byte](../../byte/) для добавления в конец текущей строки. |

### ReturnValue

Самоссылка.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
