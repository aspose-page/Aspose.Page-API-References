...
Реализует перевод оператора 'is'. Специализация для типов-оберток исключений.
...
Всегда возвращает false, так как типы несовместимы.
...
Реализует перевод оператора 'is'. Специализация значимых типов, упакованных в интерфейсы.
...
Реализует трансляцию оператора 'is'. Специализация для упаковываемых (value) типов, которые именно таковыми являются.
...
Реализует трансляцию оператора 'is'. Специализация для указательных типов, оптимизированных для 'final' классов.
