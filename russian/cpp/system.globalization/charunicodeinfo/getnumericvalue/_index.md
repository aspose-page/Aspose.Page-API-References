---
title: "System::Globalization::CharUnicodeInfo::GetNumericValue метод"
linktitle: "GetNumericValue"
second_title: "Aspose.Page для C++"
description: "System::Globalization::CharUnicodeInfo::GetNumericValue метод. Получает числовое значение, связанное с указанным символом, в C++."
type: docs
weight: 300
url: /ru/cpp/system.globalization/charunicodeinfo/getnumericvalue/
---
## CharUnicodeInfo::GetNumericValue(char16_t) method


Получает числовое значение, связанное с указанным символом.

```cpp
static double System::Globalization::CharUnicodeInfo::GetNumericValue(char16_t ch)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| ch | char16_t | Unicode-символ. |

### ReturnValue

Числовое значение или -1, если указанный символ не является числовым символом.

## См. также

* Class [CharUnicodeInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Page for C++](../../../)
## CharUnicodeInfo::GetNumericValue(const String\&, int) method


Получает числовое значение, связанное с символом по указанному индексу строки.

```cpp
static double System::Globalization::CharUnicodeInfo::GetNumericValue(const String &str, int index)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | Строка, содержащая символ Unicode. |
| индекс | int | Индекс символа Unicode. |

### ReturnValue

Числовое значение или -1, если указанный символ не является числовым символом.

## См. также

* Class [String](../../../system/string/)
* Class [CharUnicodeInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Page for C++](../../../)
