...
description: "System::Xml::XmlNode::get_ParentNode метод. Возвращает родительский узел этого узла (для узлов, которые могут иметь родителей) в C++."
type: docs
weight: 2100
...
Если узел только что создан и ещё не добавлен в дерево, или если он был удалён из дерева, родитель — **nullptr**. Для всех остальных узлов возвращаемое значение зависит от [XmlNode::get_NodeType](../get_nodetype/) узла. Следующая таблица описывает возможные возвращаемые значения для метода **get_NodeType**. |||
|-|-|
