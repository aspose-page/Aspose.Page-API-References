...
description: "System::Xml::XmlReader::ReadElementString метод. Считывает элемент, содержащий только текст. Однако рекомендуется использовать метод XmlReader::ReadElementContentAsString, поскольку он обеспечивает более простой способ выполнения этой операции в C++."
type: docs
weight: 6400
...
Считывает элемент, содержащий только текст. Однако рекомендуется использовать метод [XmlReader::ReadElementContentAsString](../readelementcontentasstring/), поскольку он обеспечивает более простой способ выполнения этой операции.
...
| --- | --- | --- |
| локальное имя | String | Локальное имя для проверки. |
