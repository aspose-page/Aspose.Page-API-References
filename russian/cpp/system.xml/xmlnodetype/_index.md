...
description: "System::Xml::XmlNodeType enum. Указывает тип узла в C++."
type: docs
weight: 6200
...
| Text | 3 | Текстовое содержимое узла. Узел [XmlNodeType::Text](./) не может иметь дочерних узлов. Он может появляться как дочерний узел у узлов [XmlNodeType::Attribute](./), [XmlNodeType::DocumentFragment](./), [XmlNodeType::Element](./) и [XmlNodeType::EntityReference](./). |
...
| Document | 9 | Объект документа, который как корень дерева документа предоставляет доступ ко всему XML‑документу. |
...
| Нотация | 12 | Нотация в объявлении типа документа (например, **<!NOTATION...>**). |
| Пробелы | 13 | Пробелы между разметкой. |
| SignificantWhitespace | 14 | Пробелы между разметкой в модели смешанного содержимого или пробелы внутри области **xml:space=\"preserve\"**. |
| EndElement | 15 | Тег завершающего элемента (например, ****). |
| EndEntity | 16 | Возвращается, когда [XmlReader](../xmlreader/) достигает конца замены сущности в результате вызова [XmlReader::ResolveEntity](../xmlreader/resolveentity/). |
