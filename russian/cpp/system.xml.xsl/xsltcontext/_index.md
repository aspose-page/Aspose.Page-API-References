...
description: "System::Xml::Xsl::XsltContext class. Инкапсулирует текущий контекст выполнения процессора Extensible Stylesheet Language for Transformations (XSLT), позволяя XML Path Language (XPath) разрешать функции, параметры и пространства имён внутри XPath‑выражений в C++."
type: docs
weight: 500
...
| Метод | Описание |
| --- | --- |
| virtual [CompareDocument](./comparedocument/)(String, String) | При переопределении в производном классе сравнивает базовые Uniform Resource Identifiers (URIs) двух документов, основываясь на порядке загрузки документов процессором XSLT (то есть классом [XslTransform](../xsltransform/)). |
...
| virtual [PreserveWhitespace](./preservewhitespace/)(SharedPtr\<System::Xml::XPath::XPathNavigator\>) | При переопределении в производном классе определяет, сохранять ли узлы пробельных символов или удалять их в данном контексте. |
