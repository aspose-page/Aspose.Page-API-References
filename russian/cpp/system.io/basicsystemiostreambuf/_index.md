...
description: "System::IO::BasicSystemIOStreamBuf класс. Представляет буфер, который оборачивает потоки, подобные System::IO::Stream, и позволяет использовать их в качестве внутреннего буфера потоков, аналогичного std::iostream, в C++."
type: docs
weight: 400
...
Представляет буфер, который оборачивает потоки, похожие на [System::IO::Stream](../stream/)-like потоки и позволяет использовать их в качестве внутреннего буфера потоков, аналогичного std::iostream.
...
| [operator=](./operator=/)(BasicSystemIOStreamBuf\&&) | Оператор перемещения присваивания. |
