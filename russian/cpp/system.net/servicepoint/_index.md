...
description: "Класс System::Net::ServicePoint. Обеспечивает управление HTTP‑соединениями. Объекты этого класса должны создаваться только с помощью функции System::MakeObject(). Никогда не создавайте экземпляр этого типа в стеке или с помощью оператора new, так как это приведёт к ошибкам времени выполнения и/или сбоям утверждений. Всегда оборачивайте этот класс в указатель System::SmartPtr и используйте этот указатель для передачи его в функции в качестве аргумента в C++."
type: docs
weight: 3100
...
| [get_IdleSince](./get_idlesince/)() | Возвращает дату и время последнего соединения с узлом. |
...
| [get_ReceiveBufferSize](./get_receivebuffersize/)() | Получает размер буфера получения. |
...
| [get_UseNagleAlgorithm](./get_usenaglealgorithm/)() | Получает значение, указывающее, используется ли алгоритм Нагла для соединений, управляемых текущим экземпляром. |
...
| [set_ReceiveBufferSize](./set_receivebuffersize/)(int32_t) | Устанавливает размер буфера получения. |
| [set_UseNagleAlgorithm](./set_usenaglealgorithm/)(bool) | Устанавливает значение, указывающее, используется ли алгоритм Нагла для соединений, управляемых текущим экземпляром. |
