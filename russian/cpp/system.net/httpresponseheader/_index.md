---
title: "Перечисление System::Net::HttpResponseHeader"
linktitle: "HttpResponseHeader"
second_title: "Aspose.Page для C++"
description: "Перечисление System::Net::HttpResponseHeader. Перечисляет заголовки HTTP-ответа в C++."
type: docs
weight: 4500
url: /ru/cpp/system.net/httpresponseheader/
---
## HttpResponseHeader enum


Перечисляет заголовки HTTP‑ответа.

```cpp
enum class HttpResponseHeader
```

### Значения

| Имя | Значение | Описание |
| --- | --- | --- |
| CacheControl | 0 | Заголовок 'Cache-Control'. |
| Connection | 1 | Заголовок 'Connection'. |
| Date | 2 | Заголовок 'Date'. |
| KeepAlive | 3 | Заголовок 'Keep-Alive'. |
| Pragma | 4 | Заголовок 'Pragma'. |
| Trailer | 5 | Заголовок 'Trailer'. |
| TransferEncoding | 6 | Заголовок 'Transfer-Encoding'. |
| Upgrade | 7 | Заголовок 'Upgrade'. |
| Via | 8 | Заголовок 'Via'. |
| Warning | 9 | Заголовок 'Warning'. |
| Allow | 10 | Заголовок 'Allow'. |
| ContentLength | 11 | Заголовок 'Content-Length'. |
| ContentType | 12 | Заголовок 'Content-Type'. |
| ContentEncoding | 13 | Заголовок 'Content-Encoding'. |
| ContentLanguage | 14 | Заголовок 'Content-Language'. |
| ContentLocation | 15 | Заголовок 'Content-Location'. |
| ContentMd5 | 16 | Заголовок 'Content-MD5'. |
| ContentRange | 17 | Заголовок 'Content-Range'. |
| Expires | 18 | Заголовок 'Expires'. |
| LastModified | 19 | Заголовок 'Last-Modified'. |
| AcceptRanges | 20 | Заголовок 'Accept-Ranges'. |
| Age | 21 | Заголовок 'Age'. |
| ETag | 22 | Заголовок 'ETag'. |
| Location | 23 | Заголовок 'Location'. |
| ProxyAuthenticate | 24 | Заголовок 'Proxy-Authenticate'. |
| RetryAfter | 25 | Заголовок 'Retry-After'. |
| Server | 26 | Заголовок 'Server'. |
| SetCookie | 27 | Заголовок 'Set-Cookie'. |
| Vary | 28 | Заголовок 'Vary'. |
| WwwAuthenticate | 29 | Заголовок 'WWW-Authenticate'. |

## См. также

* Namespace [System::Net](../)
* Library [Aspose.Page for C++](../../)
