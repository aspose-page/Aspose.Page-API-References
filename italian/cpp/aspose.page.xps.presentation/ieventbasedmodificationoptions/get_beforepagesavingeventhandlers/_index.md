---
title: "Aspose::Page::XPS::Presentation::IEventBasedModificationOptions::get_BeforePageSavingEventHandlers method"
linktitle: "get_BeforePageSavingEventHandlers"
second_title: "Aspose.Page per C++"
description: "Aspose::Page::XPS::Presentation::IEventBasedModificationOptions::get_BeforePageSavingEventHandlers method. La raccolta di gestori di eventi che esegue modifiche a una pagina XPS appena prima del salvataggio in C++."
type: docs
weight: 100
url: /it/cpp/aspose.page.xps.presentation/ieventbasedmodificationoptions/get_beforepagesavingeventhandlers/
---
## IEventBasedModificationOptions::get_BeforePageSavingEventHandlers method


La raccolta di gestori di eventi che esegue modifiche a una pagina [XPS](../../../aspose.page.xps/) poco prima che venga salvata.

```cpp
virtual System::SharedPtr<System::Collections::Generic::IList<System::SharedPtr<Features::EventBasedModifications::BeforePageSavingEventHandler>>> Aspose::Page::XPS::Presentation::IEventBasedModificationOptions::get_BeforePageSavingEventHandlers()=0
```

## Vedi anche

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IList](../../../system.collections.generic/ilist/)
* Class [BeforePageSavingEventHandler](../../../aspose.page.xps.features.eventbasedmodifications/beforepagesavingeventhandler/)
* Class [IEventBasedModificationOptions](../)
* Namespace [Aspose::Page::XPS::Presentation](../../)
* Library [Aspose.Page for C++](../../../)
