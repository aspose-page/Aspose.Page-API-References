---
title: GetPropertySize
second_title: Aspose.Page per riferimento all'API .NET
description: Ottiene il valore della proprietà della dimensione.
type: docs
weight: 100
url: /it/net/aspose.page/userproperties/getpropertysize/
---
## GetPropertySize(string) {#getpropertysize}

Ottiene il valore della proprietà della dimensione.

```csharp
public virtual Size GetPropertySize(string key)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| key | String | Il nome della proprietà. |

### Valore di ritorno

Valore della proprietà.

### Guarda anche

* class [UserProperties](../../userproperties)
* spazio dei nomi [Aspose.Page](../../userproperties)
* assemblea [Aspose.Page](../../../)

---

## GetPropertySize(string, Size) {#getpropertysize_1}

Ottiene il valore della proprietà della dimensione. Se la proprietà richiesta è assente, restituisce il valore predefinito fornito.

```csharp
public virtual Size GetPropertySize(string key, Size def)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| key | String | Il nome della proprietà. |
| def | Size | Valore predefinito della proprietà. |

### Valore di ritorno

Valore della proprietà.

### Guarda anche

* class [UserProperties](../../userproperties)
* spazio dei nomi [Aspose.Page](../../userproperties)
* assemblea [Aspose.Page](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Page.dll -->
