---
title: GetPropertyColor
second_title: Aspose.Page per riferimento all'API .NET
description: Ottiene il valore della proprietà del colore.
type: docs
weight: 40
url: /it/net/aspose.page/userproperties/getpropertycolor/
---
## GetPropertyColor(string) {#getpropertycolor}

Ottiene il valore della proprietà del colore.

```csharp
public virtual Color GetPropertyColor(string key)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| key | String | Il nome della proprietà. |

### Valore di ritorno

Valore della proprietà.

### Guarda anche

* class [UserProperties](../../userproperties)
* spazio dei nomi [Aspose.Page](../../userproperties)
* assemblea [Aspose.Page](../../../)

---

## GetPropertyColor(string, Color) {#getpropertycolor_1}

Ottiene il valore della proprietà del colore. Se la proprietà richiesta è assente, restituisce il valore predefinito fornito.

```csharp
public virtual Color GetPropertyColor(string key, Color def)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| key | String | Il nome della proprietà. |
| def | Color | Valore predefinito della proprietà. |

### Valore di ritorno

Valore della proprietà.

### Guarda anche

* class [UserProperties](../../userproperties)
* spazio dei nomi [Aspose.Page](../../userproperties)
* assemblea [Aspose.Page](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Page.dll -->
