---
title: PsDocument.Merge
second_title: Aspose.Page per riferimento all'API .NET
description: PsDocument metodo. Unisce i file PS/EPS a un dispositivo.
type: docs
weight: 160
url: /it/net/aspose.page.eps/psdocument/merge/
---
## PsDocument.Merge method

Unisce i file PS/EPS a un dispositivo.

```csharp
public void Merge(string[] filesForMerge, Device device, SaveOptions options)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| filesForMerge | String[] | File PS/EPS per l'unione con questo file in un dispositivo di output. |
| device | Device | Un dispositivo di output. |
| options | SaveOptions | Contiene flag che specificano l'output degli errori generati durante la conversione. |

### Guarda anche

* class [Device](../../../aspose.page/device/)
* class [SaveOptions](../../../aspose.page/saveoptions/)
* class [PsDocument](../)
* spazio dei nomi [Aspose.Page.EPS](../../psdocument/)
* assemblea [Aspose.Page](../../../)


