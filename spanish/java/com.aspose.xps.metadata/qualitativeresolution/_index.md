---
title: "PageResolution.QualitativeResolution"
second_title: "Referencia de API de Aspose.Page para Java"
description: "Define constantes para el valor de la propiedad puntuada QualitativeResolution."
type: docs
weight: 11
url: /es/java/com.aspose.xps.metadata/pageresolution.qualitativeresolution/
---
**Inheritance:**
java.lang.Object, [com.aspose.xps.metadata.PrintTicketElement](../../com.aspose.xps.metadata/printticketelement), [com.aspose.xps.metadata.CompositePrintTicketElement](../../com.aspose.xps.metadata/compositeprintticketelement), [com.aspose.xps.metadata.ScoredProperty](../../com.aspose.xps.metadata/scoredproperty)

**All Implemented Interfaces:**
[com.aspose.xps.metadata.PageResolution.IPageResolutionOptionItem](../../com.aspose.xps.metadata/ipageresolutionoptionitem)
```
public static final class PageResolution.QualitativeResolution extends ScoredProperty implements PageResolution.IPageResolutionOptionItem
```

Define constantes para el valor de propiedad puntuado QualitativeResolution.
## Campos

| Campo | Descripción |
| --- | --- |
| [Default](#Default) | Valor predeterminado. |
| [Draft](#Draft) | Valor provisional. |
| [High](#High) | Valor alto. |
| [Normal](#Normal) | Valor normal. |
| [Other](#Other) | Otro valor. |
## Métodos

| Método | Descripción |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [getName()](#getName--) | Obtiene el nombre del elemento. |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### Default {#Default}
```
public static PageResolution.QualitativeResolution Default
```


Valor predeterminado.

### Draft {#Draft}
```
public static PageResolution.QualitativeResolution Draft
```


Valor provisional.

### High {#High}
```
public static PageResolution.QualitativeResolution High
```


Valor alto.

### Normal {#Normal}
```
public static PageResolution.QualitativeResolution Normal
```


Valor normal.

### Other {#Other}
```
public static PageResolution.QualitativeResolution Other
```


Otro valor.

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getName() {#getName--}
```
public String getName()
```


Obtiene el nombre del elemento.

**Returns:**
java.lang.String
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

