---
title: Class JobDeviceLanguage
second_title: Aspose.Page for .NET API Referansı
description: Aspose.Page.XPS.XpsMetadata.JobDeviceLanguage sınıf. Sürücüden fiziksel aygıta veri göndermek için desteklenen aygıt dillerini tanımlar. Buna genellikle Sayfa Açıklama Dili denir. Bu anahtar sözcük sürücü ve fiziksel aygıt tarafından hangi sayfanın description dilinin desteklendiğini tanımlar. https//docs.microsoft.com/enus/windows/win32/printdocs/jobdevicelanguage
type: docs
weight: 1240
url: /tr/net/aspose.page.xps.xpsmetadata/jobdevicelanguage/
---
## JobDeviceLanguage class

Sürücüden fiziksel aygıta veri göndermek için desteklenen aygıt dillerini tanımlar. Buna genellikle "Sayfa Açıklama Dili" denir. Bu anahtar sözcük, sürücü ve fiziksel aygıt tarafından hangi sayfanın description dilinin desteklendiğini tanımlar. https://docs.microsoft.com/en-us/windows/win32/printdocs/jobdevicelanguage

```csharp
public sealed class JobDeviceLanguage : Feature, IJobPrintTicketItem
```

## yapıcılar

| İsim | Tanım |
| --- | --- |
| [JobDeviceLanguage](jobdevicelanguage/)(params JobDeviceLanguageOption[]) | Yeni bir örnek oluşturur. |

## Özellikleri

| İsim | Tanım |
| --- | --- |
| [Name](../../aspose.page.xps.xpsmetadata/printticketelement/name/) { get; } | Öğe adını alır. |

## yöntemler

| İsim | Tanım |
| --- | --- |
| [Add](../../aspose.page.xps.xpsmetadata/feature/add/)(params IFeatureItem[]) | Bu özelliğin öğe listesinin sonuna bir öğe listesi ekler. Her biri bir olmalıdır[`Feature`](../feature/) , BİR[`Option`](../option/) veya bir[`Property`](../property/) örnek. |

## Diğer_Üyeler

| İsim | Tanım |
| --- | --- |
| interface [IJobDeviceLanguageOptionItem](jobdevicelanguage.ijobdevicelanguageoptionitem/) | Herhangi birinin arayüzü[`JobDeviceLanguageOption`](../jobdevicelanguage.jobdevicelanguageoption/) öğe. |
| class [JobDeviceLanguageOption](jobdevicelanguage.jobdevicelanguageoption/) | `JobDeviceLanguage` özellik seçenekleri. |

### Ayrıca bakınız

* class [Feature](../feature/)
* interface [IJobPrintTicketItem](../ijobprintticketitem/)
* ad alanı [Aspose.Page.XPS.XpsMetadata](../../aspose.page.xps.xpsmetadata/)
* toplantı [Aspose.Page](../../)


