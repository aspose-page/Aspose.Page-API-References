---
title: "System::WeakReference< T >::operator!= 메서드"
linktitle: "operator!="
second_title: "C++용 Aspose.Page"
description: "System::WeakReference< T >::operator!= 메서드. C++에서 WeakReference 클래스의 다른 인스턴스와 참조된 객체를 비교합니다."
type: docs
weight: 200
url: /ko/cpp/system/weakreference_t_/operator!=/
---
## WeakReference< T >::operator!=(const WeakReference\<T\>\&) const method


참조된 객체를 다른 인스턴스 [WeakReference](../../weakreference/) 클래스와 비교합니다.

```cpp
bool System::WeakReference<T>::operator!=(const WeakReference<T> &other) const
```


| 매개변수 | 형식 | 설명 |
| --- | --- | --- |
| other | const WeakReference\<T\>\& | [Object](../../object/)와 비교합니다. |

### ReturnValue

비교된 객체가 서로 다른 객체를 참조하면 true, 객체가 동일하면 false.

## 또 보기

* Class [WeakReference](../../weakreference/)
* Class [WeakReference< T >](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## WeakReference< T >::operator!=(std::nullptr_t) const method


참조된 객체가 null이 아닌지 확인합니다.

```cpp
bool System::WeakReference<T>::operator!=(std::nullptr_t) const
```


### ReturnValue

참조된 객체가 null이 아니면 true, 그렇지 않으면 false.

## 또 보기

* Class [WeakReference< T >](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
