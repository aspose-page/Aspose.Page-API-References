...
description: Aspose.Page.SaveOptions 수업. 이 클래스에는 변환 프로세스 관리에 필요한 옵션이 포함되어 있습니다.
type: docs
weight: 300
url: /ko/net/aspose.page/saveoptions/
---
## SaveOptions class

이 클래스에는 변환 프로세스 관리에 필요한 옵션이 포함되어 있습니다.
...
| 이름 | 설명 |
| --- | --- |
| [SaveOptions](saveoptions/#constructor)() | 의 새 인스턴스를 초기화합니다.`SaveOptions` 플래그에 대한 기본값 가 있는 클래스!:SuppressErrors (참) 및[`Debug`](./debug/) (거짓). |
| [SaveOptions](saveoptions/#constructor_1)(bool) | 의 새 인스턴스를 초기화합니다.`SaveOptions` 플래그에 대한 기본값이 있는 클래스[`Debug`](./debug/) (거짓). |
...
| virtual [SupressErrors](../../aspose.page/saveoptions/supresserrors/) { get; set; } | 오류를 억제해야 하는지 여부를 지정합니다. true인 경우 억제된 오류가 다음에 추가됩니다.[`Exceptions`](./exceptions/) list. 거짓이면 첫 번째 오류가 프로그램을 종료합니다. |
