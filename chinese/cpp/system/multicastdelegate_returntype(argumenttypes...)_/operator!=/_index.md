...
description: "System::MulticastDelegate< ReturnType(ArgumentTypes...)>::operator!= 方法。确定两个 MulticastDelegate 实例（当前对象和指定对象）在 C++ 中是否不相等。"
type: docs
weight: 1300
...
确定两个 MulticastDelegate 实例——当前对象和指定对象——是否不相等。
...
| Parameter | Type | 描述 |
| --- | --- | --- |
| 其他 | const MulticastDelegate\& | 用于比较的 MulticastDelegate 对象 |

### ReturnValue

如果两个对象表示相同的委托集合则为 true，否则为 false
