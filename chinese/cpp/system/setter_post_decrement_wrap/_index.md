---
title: "System::setter_post_decrement_wrap 方法"
linktitle: "setter_post_decrement_wrap"
second_title: "Aspose.Page 适用于 C++"
description: "System::setter_post_decrement_wrap 方法。翻译器将 C#''s 的后递减表达式（针对具有 setter 和 getter 的实例属性）转换为在 C++ 中调用此函数（针对 const getter 的重载）。"
type: docs
weight: 37600
url: /zh/cpp/system/setter_post_decrement_wrap/
---
## System::setter_post_decrement_wrap(Host *const, T(HostConstGet::*)() const, void(HostSet::*)(T)) method


翻译器将 C#'s 的后递减表达式（针对具有 setter 和 getter 的实例属性）转换为调用此函数（针对 const getter 的重载）。
...
翻译器将 C#'s 的后递减表达式（针对具有 setter 和 getter 的实例属性）转换为调用此函数（针对非 const getter 的重载）。
...
翻译器将 C#'s 的后递减表达式（针对具有 setter 和 getter 的类属性）转换为调用此函数。
