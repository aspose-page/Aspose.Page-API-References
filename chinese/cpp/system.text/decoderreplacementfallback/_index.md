...
description: "System::Text::DecoderReplacementFallback 类。提供将错误符号替换为存根的回退策略。此类的对象应仅通过 System::MakeObject() 函数分配。切勿在栈上或使用 operator new 创建此类型的实例，因为这会导致运行时错误和/或断言故障。始终将此类包装在 System::SmartPtr 指针中，并使用该指针将其作为参数传递给 C++ 中的函数。"
type: docs
weight: 700
...
| [DecoderReplacementFallback](./decoderreplacementfallback/)() | 构造函数使用默认的 "?" 替换字符串。 |
