---
title: XpsDocument.Merge
second_title: Aspose.Page for .NET API 参考
description: XpsDocument 方法. 使用将 XPS 文档合并为 PDFDevice实例.
type: docs
weight: 450
url: /zh/net/aspose.page.xps/xpsdocument/merge/
---
## Merge(string[], Device, SaveOptions) {#merge}

使用将 XPS 文档合并为 PDF[`Device`](../../../aspose.page/device/)实例.

```csharp
public void Merge(string[] filesForMerge, Device device, SaveOptions options)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filesForMerge | String[] | 用于与本文档合并到输出设备的 XPS 文件。 |
| device | Device | 这[`Device`](../../../aspose.page/device/)实例。 |
| options | SaveOptions | 文档保存选项。 |

### 也可以看看

* class [Device](../../../aspose.page/device/)
* class [SaveOptions](../../../aspose.page/saveoptions/)
* class [XpsDocument](../)
* 命名空间 [Aspose.Page.XPS](../../xpsdocument/)
* 部件 [Aspose.Page](../../../)

---

## Merge(string[], Stream) {#merge_1}

将多个 XPS 文件合并为一个 XPS 文档。

```csharp
public void Merge(string[] filesForMerge, Stream outStream)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filesForMerge | String[] | 用于与本文档合并的 XPS 文件。 |
| outStream | Stream | 保存合并的 XPS 文档的输出流。 |

### 也可以看看

* class [XpsDocument](../)
* 命名空间 [Aspose.Page.XPS](../../xpsdocument/)
* 部件 [Aspose.Page](../../../)


