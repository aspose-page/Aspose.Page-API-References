---
title: Class XpsVisualBrush
second_title: Aspose.Page for .NET API 参考
description: Aspose.Page.XPS.XpsModel.XpsVisualBrush 班级. 封装 VisualBrush 属性元素特性的类 该元素用于用绘图填充区域
type: docs
weight: 3420
url: /zh/net/aspose.page.xps.xpsmodel/xpsvisualbrush/
---
## XpsVisualBrush class

封装 VisualBrush 属性元素特性的类。 该元素用于用绘图填充区域。

```csharp
public sealed class XpsVisualBrush : XpsTilingBrush
```

## 特性

| 姓名 | 描述 |
| --- | --- |
| [Opacity](../../aspose.page.xps.xpsmodel/xpsbrush/opacity/) { get; set; } | 返回/设置定义画笔填充的均匀透明度的值。 |
| [TileMode](../../aspose.page.xps.xpsmodel/xpstilingbrush/tilemode/) { get; set; } | 返回/设置指定如何在填充几何中执行平铺的值。 |
| [Transform](../../aspose.page.xps.xpsmodel/xpstransformablebrush/transform/) { get; set; } | 返回/设置应用于画笔坐标空间的矩阵变换。 Transform 属性与当前有效渲染 transform 连接，以产生画笔局部的有效渲染变换。 brush 的视口使用本地有效渲染变换进行变换。 |
| [Viewbox](../../aspose.page.xps.xpsmodel/xpstilingbrush/viewbox/) { get; set; } | 返回/设置要映射到视口的画笔源内容区域。 |
| [Viewport](../../aspose.page.xps.xpsmodel/xpstilingbrush/viewport/) { get; set; } | 返回/设置第一个画笔块的位置和尺寸。后续图块相对于此图块定位 ，由图块模式指定。 |
| [Visual](../../aspose.page.xps.xpsmodel/xpsvisualbrush/visual/) { get; } | 返回/设置用于绘制画笔源内容的路径、字形或画布元素。 |

## 方法

| 姓名 | 描述 |
| --- | --- |
| [Clone](../../aspose.page.xps.xpsmodel/xpsvisualbrush/clone/)() | 克隆这个视觉画笔。 |
| [SetVisual](../../aspose.page.xps.xpsmodel/xpsvisualbrush/setvisual/)(XpsContentElement) | 集*visual*作为视觉画笔的视觉元素. |

### 也可以看看

* class [XpsTilingBrush](../xpstilingbrush/)
* 命名空间 [Aspose.Page.XPS.XpsModel](../../aspose.page.xps.xpsmodel/)
* 部件 [Aspose.Page](../../)


