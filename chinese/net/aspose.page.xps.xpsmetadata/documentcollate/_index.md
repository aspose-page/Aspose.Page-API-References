...
description: 描述输出的整理特性每个单独文档中的所有页面都进行了整理 DocumentCollate 和 JobCollateAlldocuments 是互斥的 这两个关键字还是只实现其中一个关键字的行为和实现留给驱动程序 https//docs.microsoft.com/enus/windows/win32/printdocs/documentcollate
type: docs
weight: 550
...
描述输出的整理特性。每个单独文档中的所有页面都进行了整理。 DocumentCollate 和 JobCollateAlldocuments 是互斥的。 这两个关键字还是只实现其中一个关键字的行为和实现留给驱动程序。 https://docs.microsoft.com/en-us/windows/win32/printdocs/documentcollate
