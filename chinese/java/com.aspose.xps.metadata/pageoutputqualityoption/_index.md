---
title: "PageOutputQuality.PageOutputQualityOption"
second_title: "Aspose.Page for Java API 参考"
description: "定义 PageOutputQuality 功能选项。"
type: docs
weight: 10
url: /zh/java/com.aspose.xps.metadata/pageoutputquality.pageoutputqualityoption/
---
**Inheritance:**
java.lang.Object, [com.aspose.xps.metadata.PrintTicketElement](../../com.aspose.xps.metadata/printticketelement), [com.aspose.xps.metadata.CompositePrintTicketElement](../../com.aspose.xps.metadata/compositeprintticketelement), [com.aspose.xps.metadata.Option](../../com.aspose.xps.metadata/option)
```
public static final class PageOutputQuality.PageOutputQualityOption extends Option
```

定义 PageOutputQuality 功能选项。
## 字段

| 字段 | 描述 |
| --- | --- |
| [Automatic](#Automatic) | 指定自动输出的意图（允许客户端自动选择最佳设置）。 |
| [Draft](#Draft) | 指定草稿输出的意图（在草稿质量的文本和图形之间取得平衡）。 |
| [Fax](#Fax) | 指定传真输出的意图（在标准传真质量之间取得平衡）。 |
| [High](#High) | 指定高质量输出的意图（在高质量的文本和图形之间取得平衡）。 |
| [Normal](#Normal) | 指定普通输出的意图（在高质量文本和图形之间保持平衡）。 |
| [Photographic](#Photographic) | 指定摄影输出的意图（图像应具有最高质量）。 |
| [Text](#Text) | 指定仅文本输出的意图（图形可能输出不佳或根本不输出）。 |
## 方法

| 方法 | 描述 |
| --- | --- |
| [add(IOptionItem[] items)](#add-com.aspose.xps.metadata.IOptionItem...-) | 将一系列项目添加到此选项的项目列表末尾。 |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [getName()](#getName--) | 获取元素名称。 |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### Automatic {#Automatic}
```
public static PageOutputQuality.PageOutputQualityOption Automatic
```


指定自动输出的意图（允许客户端自动选择最佳设置）。

### Draft {#Draft}
```
public static PageOutputQuality.PageOutputQualityOption Draft
```


指定草稿输出的意图（在草稿质量的文本和图形之间取得平衡）。

### Fax {#Fax}
```
public static PageOutputQuality.PageOutputQualityOption Fax
```


指定传真输出的意图（在标准传真质量之间取得平衡）。

### High {#High}
```
public static PageOutputQuality.PageOutputQualityOption High
```


指定高质量输出的意图（在高质量的文本和图形之间取得平衡）。

### Normal {#Normal}
```
public static PageOutputQuality.PageOutputQualityOption Normal
```


指定普通输出的意图（在高质量文本和图形之间保持平衡）。

### Photographic {#Photographic}
```
public static PageOutputQuality.PageOutputQualityOption Photographic
```


指定摄影输出的意图（图像应具有最高质量）。

### Text {#Text}
```
public static PageOutputQuality.PageOutputQualityOption Text
```


指定仅文本输出的意图（图形可能输出不佳或根本不输出）。

### add(IOptionItem[] items) {#add-com.aspose.xps.metadata.IOptionItem...-}
```
public void add(IOptionItem[] items)
```


将一系列项目添加到此选项的项目列表末尾。每个项目必须是 ScoredProperty 或 Property 实例。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| items | [IOptionItem\[\]](../../com.aspose.xps.metadata/ioptionitem) | 要添加的项目列表。 |

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getName() {#getName--}
```
public String getName()
```


获取元素名称。

**Returns:**
java.lang.String
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

