---
title: "XpsPageLinkTarget"
second_title: "مرجع Aspose.Page لـ Java API"
description: "الفئة التي تُجمل هدف الارتباط التشعبي للصفحة."
type: docs
weight: 39
url: /ar/java/com.aspose.xps/xpspagelinktarget/
---
**Inheritance:**
java.lang.Object, [com.aspose.xps.XpsHyperlinkTarget](../../com.aspose.xps/xpshyperlinktarget)
```
public final class XpsPageLinkTarget extends XpsHyperlinkTarget
```

الفئة التي تُجمل هدف الارتباط التشعبي للصفحة.
## المنشئات

| منشئ | الوصف |
| --- | --- |
| [XpsPageLinkTarget(int targetPageNumber)](#XpsPageLinkTarget-int-) | ينشئ المثيل الجديد. |
## الطرق

| طريقة | الوصف |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [getTargetPageNumber()](#getTargetPageNumber--) | يحصل على رقم الصفحة التي يشير إليها عنصر XPS الأب. |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### XpsPageLinkTarget(int targetPageNumber) {#XpsPageLinkTarget-int-}
```
public XpsPageLinkTarget(int targetPageNumber)
```


ينشئ المثيل الجديد.

**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| targetPageNumber | int | رقم الصفحة المطلق داخل مستند XPS الكامل (تسلسل المستند الثابت). |

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getTargetPageNumber() {#getTargetPageNumber--}
```
public int getTargetPageNumber()
```


يحصل على رقم الصفحة التي يشير إليها عنصر XPS الأب.

**Returns:**
int - رقم الصفحة التي يشير إليها عنصر XPS الأب.
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

