---
title: GetPropertyMargins
second_title: Aspose.Page لمرجع NET API
description: الحصول على قيمة خاصية الهوامش .
type: docs
weight: 80
url: /ar/net/aspose.page/userproperties/getpropertymargins/
---
## GetPropertyMargins(string) {#getpropertymargins}

الحصول على قيمة خاصية الهوامش .

```csharp
public virtual Margins GetPropertyMargins(string key)
```

| معامل | يكتب | وصف |
| --- | --- | --- |
| key | String | اسم الملكية. |

### قيمة الإرجاع

قيمة العقار.

### أنظر أيضا

* class [Margins](../../margins)
* class [UserProperties](../../userproperties)
* مساحة الاسم [Aspose.Page](../../userproperties)
* المجسم [Aspose.Page](../../../)

---

## GetPropertyMargins(string, Margins) {#getpropertymargins_1}

يحصل على قيمة خاصية الهوامش. إذا كانت الخاصية المطلوبة غير موجودة ، فإن المرتجعات تقدم القيمة الافتراضية.

```csharp
public virtual Margins GetPropertyMargins(string key, Margins def)
```

| معامل | يكتب | وصف |
| --- | --- | --- |
| key | String | اسم الملكية. |
| def | Margins | القيمة الافتراضية للممتلكات. |

### قيمة الإرجاع

قيمة العقار.

### أنظر أيضا

* class [Margins](../../margins)
* class [UserProperties](../../userproperties)
* مساحة الاسم [Aspose.Page](../../userproperties)
* المجسم [Aspose.Page](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Page.dll -->
