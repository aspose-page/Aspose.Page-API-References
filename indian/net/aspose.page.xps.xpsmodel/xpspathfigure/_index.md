---
title: Class XpsPathFigure
second_title: .NET API संदर्भ के लिए Aspose.Page
description: Aspose.Page.XPS.XpsModel.XpsPathFigure कक्ष. क्लस इनकैप्सुलेटंग पथफगर एलमेंट फचर्स यह एलमेंट एक य अधक लइन य कर्व सेगमेंट के सेट से बन है
type: docs
weight: 3260
url: /hi/net/aspose.page.xps.xpsmodel/xpspathfigure/
---
## XpsPathFigure class

क्लास इनकैप्सुलेटिंग पाथफिगर एलिमेंट फीचर्स। यह एलिमेंट एक या अधिक लाइन या कर्व सेगमेंट के सेट से बना है।

```csharp
public class XpsPathFigure : XpsArray<XpsPathSegment>
```

## गुण

| नाम | विवरण |
| --- | --- |
| [Count](../../aspose.page.xps.xpsmodel/xpsarray-1/count/) { get; } |  |
| [IsClosed](../../aspose.page.xps.xpsmodel/xpspathfigure/isclosed/) { get; set; } | मान लौटाता है/सेट करता है जो बताता है कि पथ आकृति बंद है या नहीं। |
| [IsFilled](../../aspose.page.xps.xpsmodel/xpspathfigure/isfilled/) { get; set; } | मान देता है/सेट करता है कि क्या पाथ आकृति कंप्यूटिंग में उपयोग की जाती है युक्त पथ ज्यामिति का क्षेत्र। |
| [Item](../../aspose.page.xps.xpsmodel/xpsarray-1/item/) { get; } |  |
| [Segments](../../aspose.page.xps.xpsmodel/xpspathfigure/segments/) { get; } | चाइल्ड पाथ सेगमेंट की वापसी सूची. |
| [StartPoint](../../aspose.page.xps.xpsmodel/xpspathfigure/startpoint/) { get; set; } | पाथ फिगर के पहले सेगमेंट के लिए शुरुआती बिंदु लौटाता है/सेट करता है. |

## तरीकों

| नाम | विवरण |
| --- | --- |
| [Add](../../aspose.page.xps.xpsmodel/xpsarray-1/add/)(XpsPathSegment) |  |
| [Clone](../../aspose.page.xps.xpsmodel/xpspathfigure/clone/)() | इस पाथ फिगर को क्लोन करता है। |
| [Insert](../../aspose.page.xps.xpsmodel/xpsarray-1/insert/)(int, XpsPathSegment) |  |
| [Remove](../../aspose.page.xps.xpsmodel/xpsarray-1/remove/)(XpsPathSegment) |  |
| [RemoveAt](../../aspose.page.xps.xpsmodel/xpsarray-1/removeat/)(int) |  |

### यह सभी देखें

* class [XpsArray&lt;T&gt;](../xpsarray-1/)
* class [XpsPathSegment](../xpspathsegment/)
* नाम स्थान [Aspose.Page.XPS.XpsModel](../../aspose.page.xps.xpsmodel/)
* सभा [Aspose.Page](../../)


