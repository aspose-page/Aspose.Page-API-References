---
title: "System::Security::Cryptography::ECDsaBotan::SignData metod"
linktitle: "SignData"
second_title: "Aspose.Page för C++"
description: "System::Security::Cryptography::ECDsaBotan::SignData metod. Beräknar hashvärdet för den angivna dataarrayen och signerar resultatet i C++."
type: docs
weight: 1100
url: /sv/cpp/system.security.cryptography/ecdsabotan/signdata/
---
## ECDsaBotan::SignData(const ByteArrayPtr\&) method


Beräknar hashvärdet för den angivna dataarrayen och signerar resultatet.

```cpp
ByteArrayPtr System::Security::Cryptography::ECDsaBotan::SignData(const ByteArrayPtr &data)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| data | const ByteArrayPtr\& | Indataarray. Returnerar ECDSA-signatur för indata. |

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## ECDsaBotan::SignData(const ByteArrayPtr\&, const HashAlgorithmName\&) method


RTTI-information.

```cpp
virtual ByteArrayPtr System::Security::Cryptography::ECDsa::SignData(const ByteArrayPtr &data, const HashAlgorithmName &hash_algorithm)
```

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## ECDsaBotan::SignData(const ByteArrayPtr\&, int32_t, int32_t) method


Beräknar hashvärdet för den angivna dataarrayen och signerar resultatet.

```cpp
ByteArrayPtr System::Security::Cryptography::ECDsaBotan::SignData(const ByteArrayPtr &data, int32_t offset, int32_t count)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| data | const ByteArrayPtr\& | Indataarray. |
| offset | int32_t | Offset i **data**. |
| count | int32_t | Antal byte att använda som indata. returnera ECDSA-signatur för indata. |

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## ECDsaBotan::SignData(const ByteArrayPtr\&, int32_t, int32_t, const HashAlgorithmName\&) method


RTTI-information.

```cpp
virtual ByteArrayPtr System::Security::Cryptography::ECDsa::SignData(const ByteArrayPtr &data, int32_t offset, int32_t count, const HashAlgorithmName &hash_algorithm)
```

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## ECDsaBotan::SignData(const StreamPtr\&) method


Beräknar hashvärdet för den angivna binära strömmen och signerar resultatet.

```cpp
ByteArrayPtr System::Security::Cryptography::ECDsaBotan::SignData(const StreamPtr &stream)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| ström | const StreamPtr\& | Binärt flöde. returnera ECDSA-signatur för indata. |

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Typedef [StreamPtr](../../../system/streamptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
## ECDsaBotan::SignData(const StreamPtr\&, const HashAlgorithmName\&) method


RTTI-information.

```cpp
virtual ByteArrayPtr System::Security::Cryptography::ECDsa::SignData(const StreamPtr &stream, const HashAlgorithmName &hash_algorithm)
```

## Se även

* Typedef [ByteArrayPtr](../../../system/bytearrayptr/)
* Typedef [StreamPtr](../../../system/streamptr/)
* Class [ECDsaBotan](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Page for C++](../../../)
