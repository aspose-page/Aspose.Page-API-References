...
| --- | --- |
| T | Typen av de objekt som metoden jämför |
## Metoder

| Metod | Beskrivning |
| --- | --- |
| [Comparison](./comparison/)(Y) | Skapar en instans av [Comparison](./)‑delegat som representerar pekaren till den angivna anropbara enheten. |
| [operator()](./operator()/)(T, T) | Invokar det anropbara objektet som pekas på av det aktuella objektet. |
## Anmärkningar
...
class MyArray
{
  // Används för att lagra array‑data.
  std::vector<T> m_data;
...
public:
  ...
  // Returnerar ett antal element som vår dynamiska array lagrar.
  size_t get_Size()
  {
    return m_data.size();
  }
