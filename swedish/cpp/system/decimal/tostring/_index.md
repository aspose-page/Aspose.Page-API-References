---
title: "System::Decimal::ToString‑metod"
linktitle: "ToString"
second_title: "Aspose.Page för C++"
description: "System::Decimal::ToString‑metod. Returnerar strängrepresentationen av värdet som representeras av objektet i C++."
type: docs
weight: 3000
url: /sv/cpp/system/decimal/tostring/
---
## Decimal::ToString() const method


Returnerar strängrepresentationen av värdet som objektet representerar.

```cpp
String System::Decimal::ToString() const
```

## Se även

* Class [String](../../string/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const Decimal\&, std::nullptr_t) const method




```cpp
String System::Decimal::ToString(const Decimal &value, std::nullptr_t) const
```

## Se även

* Class [String](../../string/)
* Class [Decimal](../)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const SharedPtr\<Globalization::CultureInfo\>\&) const method




```cpp
String System::Decimal::ToString(const SharedPtr<Globalization::CultureInfo> &culture) const
```

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [CultureInfo](../../../system.globalization/cultureinfo/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const SharedPtr\<Globalization::NumberFormatInfo\>\&) const method




```cpp
String System::Decimal::ToString(const SharedPtr<Globalization::NumberFormatInfo> &nfi) const
```

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [NumberFormatInfo](../../../system.globalization/numberformatinfo/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const SharedPtr\<IFormatProvider\>\&) const method


Konverterar det aktuella objektet till en sträng med hjälp av kulturspecifik formatinformation.

```cpp
String System::Decimal::ToString(const SharedPtr<IFormatProvider> &provider) const
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| provider | const SharedPtr\<IFormatProvider\>\& | Det [IFormatProvider](../../iformatprovider/)-objektet som tillhandahåller kultur‑specifik formatinformation. |

### ReturnValue

Strängrepresentationen av det aktuella objektet.

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [IFormatProvider](../../iformatprovider/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const String\&, const SharedPtr\<Globalization::CultureInfo\>\&) const method




```cpp
String System::Decimal::ToString(const String &format, const SharedPtr<Globalization::CultureInfo> &culture) const
```

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [CultureInfo](../../../system.globalization/cultureinfo/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const String\&, const SharedPtr\<Globalization::NumberFormatInfo\>\&) const method




```cpp
String System::Decimal::ToString(const String &format, const SharedPtr<Globalization::NumberFormatInfo> &nfi) const
```

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [NumberFormatInfo](../../../system.globalization/numberformatinfo/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const String\&, const SharedPtr\<IFormatProvider\>\&) const method


Konverterar det aktuella objektet till dess strängrepresentation med hjälp av det angivna strängformatet och kultur‑specifik formatinformation som tillhandahålls av det angivna [IFormatProvider](../../iformatprovider/)‑objektet.

```cpp
String System::Decimal::ToString(const String &format, const SharedPtr<IFormatProvider> &provider) const
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| format | const String\& | Strängformatet. |
| provider | const SharedPtr\<IFormatProvider\>\& | Det [IFormatProvider](../../iformatprovider/)-objektet som tillhandahåller kultur‑specifik formatinformation. |

### ReturnValue

Strängrepresentationen av det aktuella objektet.

## Se även

* Class [String](../../string/)
* Typedef [SharedPtr](../../sharedptr/)
* Class [IFormatProvider](../../iformatprovider/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
## Decimal::ToString(const String\&, std::nullptr_t) const method




```cpp
String System::Decimal::ToString(const String &format, std::nullptr_t=nullptr) const
```

## Se även

* Class [String](../../string/)
* Class [Decimal](../)
* Namespace [System](../../)
* Library [Aspose.Page for C++](../../../)
