---
title: "System::Threading::Thread::Thread konstruktor"
linktitle: "Thread"
second_title: "Aspose.Page för C++"
description: "System::Threading::Thread::Thread konstruktor. Konstruktor i C++."
type: docs
weight: 100
url: /sv/cpp/system.threading/thread/thread/
---
## Thread::Thread() constructor


Konstruktor.

```cpp
System::Threading::Thread::Thread()
```

## Se även

* Class [Thread](../)
* Namespace [System::Threading](../../)
* Library [Aspose.Page for C++](../../../)
## Thread::Thread(ParameterizedThreadStart) constructor


Konstruktor.

```cpp
System::Threading::Thread::Thread(ParameterizedThreadStart thread_function)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| thread_function | ParameterizedThreadStart | Funktion som ska köras av tråden. |

## Se även

* Typedef [ParameterizedThreadStart](../../parameterizedthreadstart/)
* Class [Thread](../)
* Namespace [System::Threading](../../)
* Library [Aspose.Page for C++](../../../)
## Thread::Thread(Thread\&) constructor


Kopieringskonstruktor.

```cpp
System::Threading::Thread::Thread(Thread &t)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| t | Thread\& | [Thread](../) att kopiera data från. |

## Se även

* Class [Thread](../)
* Class [Thread](../)
* Namespace [System::Threading](../../)
* Library [Aspose.Page for C++](../../../)
## Thread::Thread(ThreadStart) constructor


Konstruktor.

```cpp
System::Threading::Thread::Thread(ThreadStart thread_function)
```


| Parameter | Type | Beskrivning |
| --- | --- | --- |
| thread_function | ThreadStart | Funktion som ska köras av tråden. |

## Se även

* Typedef [ThreadStart](../../threadstart/)
* Class [Thread](../)
* Namespace [System::Threading](../../)
* Library [Aspose.Page for C++](../../../)
