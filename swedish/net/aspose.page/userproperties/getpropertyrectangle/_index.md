---
title: GetPropertyRectangle
second_title: Aspose.Page för .NET API-referens
description: Får rektangelegenskapsvärde.
type: docs
weight: 90
url: /sv/net/aspose.page/userproperties/getpropertyrectangle/
---
## GetPropertyRectangle(string) {#getpropertyrectangle}

Får rektangelegenskapsvärde.

```csharp
public virtual RectangleF GetPropertyRectangle(string key)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| key | String | Egendomens namn. |

### Returvärde

Fastighetsvärde.

### Se även

* class [UserProperties](../../userproperties)
* namnutrymme [Aspose.Page](../../userproperties)
* hopsättning [Aspose.Page](../../../)

---

## GetPropertyRectangle(string, RectangleF) {#getpropertyrectangle_1}

Får rektangelegenskapsvärde. Om den begärda egenskapen saknas, returnerar det angivna standardvärdet.

```csharp
public virtual RectangleF GetPropertyRectangle(string key, RectangleF def)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| key | String | Egendomens namn. |
| def | RectangleF | Standardvärde för egendom. |

### Returvärde

Fastighetsvärde.

### Se även

* class [UserProperties](../../userproperties)
* namnutrymme [Aspose.Page](../../userproperties)
* hopsättning [Aspose.Page](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Page.dll -->
