...
description: "System::Threading::ThreadPool class。スレッドプール API で、ジョブをキューにプッシュし、ワーカースレッドのプールがそれを取得できるようにします。これはインスタンスサービスを持たない静的型です。C++ ではいかなる手段でもインスタンスを作成すべきではありません。"
type: docs
weight: 1300
url: /ja/cpp/system.threading/threadpool/
---
## ThreadPool class
...
| メソッド | 説明 |
| --- | --- |
| static [GetAvailableThreads](./getavailablethreads/)(int\&, int\&) | 利用可能なスレッド数を取得します。 |
| static [GetInstance](./getinstance/)() | RTTI 情報。 |
| static [GetMaxThreads](./getmaxthreads/)(int\&, int\&) | 同時実行可能な最大スレッド数を取得します。 |
| static [GetMinThreads](./getminthreads/)(int\&, int\&) | プールが作成する最小スレッド数を取得します。 |
| static [JoinAllThreads](./joinallthreads/)() | 所有するすべてのスレッドに参加します。無限に待ちます。 |
| [operator=](./operator=/)(const ThreadPool\&) | コピーはできません。 |
| static [QueueUserWorkItem](./queueuserworkitem/)(WaitCallback) | パラメータなしのコールバックを伴う作業項目をキューに入れます。 |
| static [QueueUserWorkItem](./queueuserworkitem/)(WaitCallback, const System::SharedPtr\<System::Object\>\&) | パラメータなしのコールバックを伴う作業項目をキューに入れます。 |
| static [SetMaxThreads](./setmaxthreads/)(int, int) | プールが所有するスレッド数を設定します。 |
| static [SetMinThreads](./setminthreads/)(int, int) | プールが所有する最小スレッド数を設定します。 |
| [ThreadPool](./threadpool/)(const ThreadPool\&) | コピーはできません。 |
## 備考
