---
title: Interface PageScaling.IPageScalingItem
second_title: Aspose.Page for .NET API リファレンス
description: Aspose.Page.XPS.XpsMetadata.PageScalingIPageScalingItem インターフェース. 任意のインターフェイスPageScaling機能アイテム.
type: docs
weight: 2490
url: /ja/net/aspose.page.xps.xpsmetadata/pagescaling.ipagescalingitem/
---
## PageScaling.IPageScalingItem interface

任意のインターフェイス[`PageScaling`](../pagescaling/)機能アイテム.

```csharp
public interface IPageScalingItem : IFeatureItem
```

### 関連項目

* interface [IFeatureItem](../ifeatureitem/)
* class [PageScaling](../pagescaling/)
* 名前空間 [Aspose.Page.XPS.XpsMetadata](../../aspose.page.xps.xpsmetadata/)
* 組み立て [Aspose.Page](../../)


