---
title: "UserProperties"
second_title: "Αναφορά API του Aspose.Page για Java"
description: "Ειδική κλάση ιδιοτήτων που επιτρέπει τον ορισμό και την επιστροφή τυποποιημένων ιδιοτήτων."
type: docs
weight: 18
url: /el/java/com.aspose.page/userproperties/
---
**Inheritance:**
java.lang.Object, java.util.Dictionary, java.util.Hashtable, java.util.Properties
```
public class UserProperties extends Properties
```

Ειδική κλάση ιδιοτήτων που επιτρέπει τον ορισμό και την επιστροφή τυποποιημένων ιδιοτήτων. Επιτρέπει επίσης τη σύνδεση δύο προεπιλεγμένων αντικειμένων ιδιοτήτων για αναζήτηση εάν αυτό το αντικείμενο ιδιοτήτων δεν περιέχει την ιδιότητα.
## Κατασκευαστές

| Κατασκευαστής | Περιγραφή |
| --- | --- |
| [UserProperties()](#UserProperties--) | Αρχικοποιεί μια κενή παρουσία της κλάσης UserProperties. |
| [UserProperties(Properties defaults)](#UserProperties-java.util.Properties-) | Αρχικοποιεί μια κλάση UserProperties με προεπιλεγμένες τιμές. |
| [UserProperties(Properties defaults, Properties altDefaults)](#UserProperties-java.util.Properties-java.util.Properties-) | Δημιουργεί το UserProperties με πίνακα defaults και altDefaults, οι οποίοι αναζητούνται με αυτή τη σειρά. |
## Μέθοδοι

| Μέθοδος | Περιγραφή |
| --- | --- |
| [clear()](#clear--) |  |
| [clone()](#clone--) |  |
| [compute(K arg0, BiFunction<? super K,? super V,? extends V> arg1)](#compute-K-java.util.function.BiFunction---super-K---super-V---extends-V--) |  |
| [compute(Object arg0, BiFunction<? super Object,? super Object,?> arg1)](#compute-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----) |  |
| [computeIfAbsent(K arg0, Function<? super K,? extends V> arg1)](#computeIfAbsent-K-java.util.function.Function---super-K---extends-V--) |  |
| [computeIfAbsent(Object arg0, Function<? super Object,?> arg1)](#computeIfAbsent-java.lang.Object-java.util.function.Function---super-java.lang.Object----) |  |
| [computeIfPresent(K arg0, BiFunction<? super K,? super V,? extends V> arg1)](#computeIfPresent-K-java.util.function.BiFunction---super-K---super-V---extends-V--) |  |
| [computeIfPresent(Object arg0, BiFunction<? super Object,? super Object,?> arg1)](#computeIfPresent-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----) |  |
| [contains(Object arg0)](#contains-java.lang.Object-) |  |
| [containsKey(Object arg0)](#containsKey-java.lang.Object-) |  |
| [containsValue(Object arg0)](#containsValue-java.lang.Object-) |  |
| [elements()](#elements--) |  |
| [entrySet()](#entrySet--) |  |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [forEach(BiConsumer<? super K,? super V> arg0)](#forEach-java.util.function.BiConsumer---super-K---super-V--) |  |
| [forEach(BiConsumer<? super Object,? super Object> arg0)](#forEach-java.util.function.BiConsumer---super-java.lang.Object---super-java.lang.Object--) |  |
| [get(Object arg0)](#get-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [getOrDefault(Object arg0, V arg1)](#getOrDefault-java.lang.Object-V-) |  |
| [getOrDefault(Object arg0, Object arg1)](#getOrDefault-java.lang.Object-java.lang.Object-) |  |
| [getProperty(String key)](#getProperty-java.lang.String-) | Αποκτά την τιμή ιδιότητας τύπου string. |
| [getProperty(String key, String def)](#getProperty-java.lang.String-java.lang.String-) | Αποκτά την τιμή ιδιότητας τύπου string. |
| [getPropertyColor(String key)](#getPropertyColor-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας χρώματος. |
| [getPropertyColor(String key, Color def)](#getPropertyColor-java.lang.String-java.awt.Color-) | Λαμβάνει την τιμή της ιδιότητας χρώματος. |
| [getPropertyDimension(String key)](#getPropertyDimension-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας διαστάσεων. |
| [getPropertyDimension(String key, Dimension def)](#getPropertyDimension-java.lang.String-java.awt.Dimension-) | Λαμβάνει την τιμή της ιδιότητας διαστάσεων. |
| [getPropertyDouble(String key)](#getPropertyDouble-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας double. |
| [getPropertyDouble(String key, double def)](#getPropertyDouble-java.lang.String-double-) | Λαμβάνει την τιμή της ιδιότητας double. |
| [getPropertyFloat(String key)](#getPropertyFloat-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας float. |
| [getPropertyFloat(String key, float def)](#getPropertyFloat-java.lang.String-float-) | Λαμβάνει την τιμή της ιδιότητας float. |
| [getPropertyInsets(String key)](#getPropertyInsets-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας insets. |
| [getPropertyInsets(String key, Insets def)](#getPropertyInsets-java.lang.String-java.awt.Insets-) | Λαμβάνει την τιμή της ιδιότητας insets. |
| [getPropertyInt(String key)](#getPropertyInt-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας integer. |
| [getPropertyInt(String key, int def)](#getPropertyInt-java.lang.String-int-) | Λαμβάνει την τιμή της ιδιότητας integer. |
| [getPropertyMatrix(String key)](#getPropertyMatrix-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας float. |
| [getPropertyMatrix(String key, AffineTransform def)](#getPropertyMatrix-java.lang.String-java.awt.geom.AffineTransform-) | Λαμβάνει την τιμή της ιδιότητας float. |
| [getPropertyRectangle(String key)](#getPropertyRectangle-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας rectangle. |
| [getPropertyRectangle(String key, Rectangle def)](#getPropertyRectangle-java.lang.String-java.awt.Rectangle-) | Λαμβάνει την τιμή της ιδιότητας rectangle. |
| [getPropertyStringArray(String key)](#getPropertyStringArray-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας string array. |
| [getPropertyStringArray(String key, String[] def)](#getPropertyStringArray-java.lang.String-java.lang.String---) | Λαμβάνει την τιμή της ιδιότητας string array. |
| [hashCode()](#hashCode--) |  |
| [isEmpty()](#isEmpty--) |  |
| [isProperty(String key)](#isProperty-java.lang.String-) | Λαμβάνει την τιμή της ιδιότητας boolean. |
| [isProperty(String key, boolean def)](#isProperty-java.lang.String-boolean-) | Λαμβάνει την τιμή της ιδιότητας boolean. |
| [keySet()](#keySet--) |  |
| [keys()](#keys--) |  |
| [list(PrintStream arg0)](#list-java.io.PrintStream-) |  |
| [list(PrintWriter arg0)](#list-java.io.PrintWriter-) |  |
| [load(InputStream arg0)](#load-java.io.InputStream-) |  |
| [load(Reader arg0)](#load-java.io.Reader-) |  |
| [loadFromXML(InputStream arg0)](#loadFromXML-java.io.InputStream-) |  |
| [merge(K arg0, V arg1, BiFunction<? super V,? super V,? extends V> arg2)](#merge-K-V-java.util.function.BiFunction---super-V---super-V---extends-V--) |  |
| [merge(Object arg0, Object arg1, BiFunction<? super Object,? super Object,?> arg2)](#merge-java.lang.Object-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [printProperties()](#printProperties--) | Εκτυπώνει όλες τις ορισμένες ιδιότητες. |
| [propertyNames()](#propertyNames--) | Επιστρέφει τα ονόματα των ιδιοτήτων. |
| [put(K arg0, V arg1)](#put-K-V-) |  |
| [put(Object arg0, Object arg1)](#put-java.lang.Object-java.lang.Object-) |  |
| [putAll(Map<? extends K,? extends V> arg0)](#putAll-java.util.Map---extends-K---extends-V--) |  |
| [putAll(Map<?,?> arg0)](#putAll-java.util.Map------) |  |
| [putIfAbsent(K arg0, V arg1)](#putIfAbsent-K-V-) |  |
| [putIfAbsent(Object arg0, Object arg1)](#putIfAbsent-java.lang.Object-java.lang.Object-) |  |
| [remove(Object arg0)](#remove-java.lang.Object-) |  |
| [remove(Object arg0, Object arg1)](#remove-java.lang.Object-java.lang.Object-) |  |
| [replace(K arg0, V arg1)](#replace-K-V-) |  |
| [replace(K arg0, V arg1, V arg2)](#replace-K-V-V-) |  |
| [replace(Object arg0, Object arg1)](#replace-java.lang.Object-java.lang.Object-) |  |
| [replace(Object arg0, Object arg1, Object arg2)](#replace-java.lang.Object-java.lang.Object-java.lang.Object-) |  |
| [replaceAll(BiFunction<? super K,? super V,? extends V> arg0)](#replaceAll-java.util.function.BiFunction---super-K---super-V---extends-V--) |  |
| [replaceAll(BiFunction<? super Object,? super Object,?> arg0)](#replaceAll-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----) |  |
| [save(OutputStream arg0, String arg1)](#save-java.io.OutputStream-java.lang.String-) |  |
| [setProperties(Properties properties)](#setProperties-java.util.Properties-) | Αντιγράφει τις ιδιότητες, συμπεριλαμβανομένων των προεπιλογών τους, σε αυτό το UserProperties |
| [setProperty(String key, boolean value)](#setProperty-java.lang.String-boolean-) | Ορίζει την τιμή της ιδιότητας boolean. |
| [setProperty(String key, double value)](#setProperty-java.lang.String-double-) | Ορίζει την τιμή της ιδιότητας double. |
| [setProperty(String key, float value)](#setProperty-java.lang.String-float-) | Ορίζει την τιμή της ιδιότητας float. |
| [setProperty(String key, int value)](#setProperty-java.lang.String-int-) | Ορίζει την τιμή της ιδιότητας integer. |
| [setProperty(String key, Color value)](#setProperty-java.lang.String-java.awt.Color-) | Ορίζει την τιμή της ιδιότητας χρώματος. |
| [setProperty(String key, Dimension value)](#setProperty-java.lang.String-java.awt.Dimension-) | Ορίζει την τιμή της ιδιότητας διαστάσεων. |
| [setProperty(String key, Insets value)](#setProperty-java.lang.String-java.awt.Insets-) | Ορίζει την τιμή της ιδιότητας insets. |
| [setProperty(String key, Rectangle value)](#setProperty-java.lang.String-java.awt.Rectangle-) | Ορίζει την τιμή της ιδιότητας rectangle. |
| [setProperty(String key, AffineTransform value)](#setProperty-java.lang.String-java.awt.geom.AffineTransform-) | Ορίζει την τιμή της ιδιότητας matrix. |
| [setProperty(String key, String value)](#setProperty-java.lang.String-java.lang.String-) | Ορίζει την τιμή της ιδιότητας string. |
| [setProperty(String key, String[] value)](#setProperty-java.lang.String-java.lang.String---) | Ορίζει την τιμή της ιδιότητας string array. |
| [setProperty(Properties properties, String key, boolean value)](#setProperty-java.util.Properties-java.lang.String-boolean-) | Ορίζει την τιμή της ιδιότητας boolean σε καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, double value)](#setProperty-java.util.Properties-java.lang.String-double-) | Ορίζει την τιμή της ιδιότητας double σε καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, float value)](#setProperty-java.util.Properties-java.lang.String-float-) | Ορίζει την τιμή της ιδιότητας float στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, int value)](#setProperty-java.util.Properties-java.lang.String-int-) | Ορίζει την τιμή της ιδιότητας integer στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, Color value)](#setProperty-java.util.Properties-java.lang.String-java.awt.Color-) | Ορίζει την τιμή της ιδιότητας color στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, Dimension value)](#setProperty-java.util.Properties-java.lang.String-java.awt.Dimension-) | Ορίζει την τιμή της ιδιότητας dimension στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, Insets value)](#setProperty-java.util.Properties-java.lang.String-java.awt.Insets-) | Ορίζει την τιμή της ιδιότητας insets στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, Rectangle value)](#setProperty-java.util.Properties-java.lang.String-java.awt.Rectangle-) | Ορίζει την τιμή της ιδιότητας rectangle στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, AffineTransform value)](#setProperty-java.util.Properties-java.lang.String-java.awt.geom.AffineTransform-) | Ορίζει την τιμή της ιδιότητας matrix στον καθορισμένο πίνακα ιδιοτήτων. |
| [setProperty(Properties properties, String key, String[] value)](#setProperty-java.util.Properties-java.lang.String-java.lang.String---) | Ορίζει την τιμή της ιδιότητας string array στον καθορισμένο πίνακα ιδιοτήτων. |
| [size()](#size--) |  |
| [store(OutputStream arg0, String arg1)](#store-java.io.OutputStream-java.lang.String-) |  |
| [store(Writer arg0, String arg1)](#store-java.io.Writer-java.lang.String-) |  |
| [storeToXML(OutputStream arg0, String arg1)](#storeToXML-java.io.OutputStream-java.lang.String-) |  |
| [storeToXML(OutputStream arg0, String arg1, String arg2)](#storeToXML-java.io.OutputStream-java.lang.String-java.lang.String-) |  |
| [storeToXML(OutputStream arg0, String arg1, Charset arg2)](#storeToXML-java.io.OutputStream-java.lang.String-java.nio.charset.Charset-) |  |
| [stringPropertyNames()](#stringPropertyNames--) |  |
| [toString()](#toString--) |  |
| [values()](#values--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### UserProperties() {#UserProperties--}
```
public UserProperties()
```


Αρχικοποιεί μια κενή παρουσία της κλάσης UserProperties.

### UserProperties(Properties defaults) {#UserProperties-java.util.Properties-}
```
public UserProperties(Properties defaults)
```


Αρχικοποιεί μια κλάση UserProperties με προεπιλεγμένες τιμές.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| προεπιλογές | java.util.Properties | Τιμές προεπιλεγμένων ιδιοτήτων. |

### UserProperties(Properties defaults, Properties altDefaults) {#UserProperties-java.util.Properties-java.util.Properties-}
```
public UserProperties(Properties defaults, Properties altDefaults)
```


Δημιουργεί το UserProperties με πίνακα defaults και altDefaults, οι οποίοι αναζητούνται με αυτή τη σειρά.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| προεπιλογές | java.util.Properties | Προεπιλεγμένες ιδιότητες. |
| altDefaults | java.util.Properties | Εναλλακτικές προεπιλεγμένες ιδιότητες. |

### clear() {#clear--}
```
public synchronized void clear()
```




### clone() {#clone--}
```
public synchronized Object clone()
```




**Returns:**
java.lang.Object
### compute(K arg0, BiFunction<? super K,? super V,? extends V> arg1) {#compute-K-java.util.function.BiFunction---super-K---super-V---extends-V--}
```
public synchronized V compute(K arg0, BiFunction<? super K,? super V,? extends V> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | java.util.function.BiFunction<? super K,? super V,? extends V> |  |

**Returns:**
V
### compute(Object arg0, BiFunction<? super Object,? super Object,?> arg1) {#compute-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----}
```
public synchronized Object compute(Object arg0, BiFunction<? super Object,? super Object,?> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.util.function.BiFunction<? super java.lang.Object,? super java.lang.Object,?> |  |

**Returns:**
java.lang.Object
### computeIfAbsent(K arg0, Function<? super K,? extends V> arg1) {#computeIfAbsent-K-java.util.function.Function---super-K---extends-V--}
```
public synchronized V computeIfAbsent(K arg0, Function<? super K,? extends V> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | java.util.function.Function<? super K,? extends V> |  |

**Returns:**
V
### computeIfAbsent(Object arg0, Function<? super Object,?> arg1) {#computeIfAbsent-java.lang.Object-java.util.function.Function---super-java.lang.Object----}
```
public synchronized Object computeIfAbsent(Object arg0, Function<? super Object,?> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.util.function.Function<? super java.lang.Object,?> |  |

**Returns:**
java.lang.Object
### computeIfPresent(K arg0, BiFunction<? super K,? super V,? extends V> arg1) {#computeIfPresent-K-java.util.function.BiFunction---super-K---super-V---extends-V--}
```
public synchronized V computeIfPresent(K arg0, BiFunction<? super K,? super V,? extends V> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | java.util.function.BiFunction<? super K,? super V,? extends V> |  |

**Returns:**
V
### computeIfPresent(Object arg0, BiFunction<? super Object,? super Object,?> arg1) {#computeIfPresent-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----}
```
public synchronized Object computeIfPresent(Object arg0, BiFunction<? super Object,? super Object,?> arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.util.function.BiFunction<? super java.lang.Object,? super java.lang.Object,?> |  |

**Returns:**
java.lang.Object
### contains(Object arg0) {#contains-java.lang.Object-}
```
public boolean contains(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### containsKey(Object arg0) {#containsKey-java.lang.Object-}
```
public boolean containsKey(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### containsValue(Object arg0) {#containsValue-java.lang.Object-}
```
public boolean containsValue(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### elements() {#elements--}
```
public Enumeration<Object> elements()
```




**Returns:**
java.util.Enumeration<java.lang.Object>
### entrySet() {#entrySet--}
```
public Set<Map.Entry<Object,Object>> entrySet()
```




**Returns:**
java.util.Set<java.util.Map.Entry<java.lang.Object,java.lang.Object>>
### equals(Object arg0) {#equals-java.lang.Object-}
```
public synchronized boolean equals(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
boolean
### forEach(BiConsumer<? super K,? super V> arg0) {#forEach-java.util.function.BiConsumer---super-K---super-V--}
```
public synchronized void forEach(BiConsumer<? super K,? super V> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.function.BiConsumer<? super K,? super V> |  |

### forEach(BiConsumer<? super Object,? super Object> arg0) {#forEach-java.util.function.BiConsumer---super-java.lang.Object---super-java.lang.Object--}
```
public synchronized void forEach(BiConsumer<? super Object,? super Object> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.function.BiConsumer<? super java.lang.Object,? super java.lang.Object> |  |

### get(Object arg0) {#get-java.lang.Object-}
```
public Object get(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
java.lang.Object
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### getOrDefault(Object arg0, V arg1) {#getOrDefault-java.lang.Object-V-}
```
public synchronized V getOrDefault(Object arg0, V arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | V |  |

**Returns:**
V
### getOrDefault(Object arg0, Object arg1) {#getOrDefault-java.lang.Object-java.lang.Object-}
```
public Object getOrDefault(Object arg0, Object arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |

**Returns:**
java.lang.Object
### getProperty(String key) {#getProperty-java.lang.String-}
```
public String getProperty(String key)
```


Αποκτά την τιμή ιδιότητας τύπου string.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.lang.String - Τιμή της ιδιότητας.
### getProperty(String key, String def) {#getProperty-java.lang.String-java.lang.String-}
```
public String getProperty(String key, String def)
```


Λαμβάνει την τιμή της ιδιότητας string. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.lang.String | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.lang.String - Τιμή της ιδιότητας.
### getPropertyColor(String key) {#getPropertyColor-java.lang.String-}
```
public Color getPropertyColor(String key)
```


Λαμβάνει την τιμή της ιδιότητας χρώματος.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.awt.Color - Τιμή της ιδιότητας.
### getPropertyColor(String key, Color def) {#getPropertyColor-java.lang.String-java.awt.Color-}
```
public Color getPropertyColor(String key, Color def)
```


Λαμβάνει την τιμή της ιδιότητας χρώματος. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.awt.Color | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.awt.Color - Τιμή της ιδιότητας.
### getPropertyDimension(String key) {#getPropertyDimension-java.lang.String-}
```
public Dimension getPropertyDimension(String key)
```


Λαμβάνει την τιμή της ιδιότητας διαστάσεων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.awt.Dimension - Τιμή της ιδιότητας.
### getPropertyDimension(String key, Dimension def) {#getPropertyDimension-java.lang.String-java.awt.Dimension-}
```
public Dimension getPropertyDimension(String key, Dimension def)
```


Λαμβάνει την τιμή της ιδιότητας διάστασης. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.awt.Dimension | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.awt.Dimension - Τιμή της ιδιότητας.
### getPropertyDouble(String key) {#getPropertyDouble-java.lang.String-}
```
public double getPropertyDouble(String key)
```


Λαμβάνει την τιμή της ιδιότητας double.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
double - Τιμή της ιδιότητας.
### getPropertyDouble(String key, double def) {#getPropertyDouble-java.lang.String-double-}
```
public double getPropertyDouble(String key, double def)
```


Λαμβάνει την τιμή της ιδιότητας double. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | double | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
double - Τιμή της ιδιότητας.
### getPropertyFloat(String key) {#getPropertyFloat-java.lang.String-}
```
public float getPropertyFloat(String key)
```


Λαμβάνει την τιμή της ιδιότητας float.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
float - Τιμή της ιδιότητας.
### getPropertyFloat(String key, float def) {#getPropertyFloat-java.lang.String-float-}
```
public float getPropertyFloat(String key, float def)
```


Λαμβάνει την τιμή της ιδιότητας float. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | float | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
float - Τιμή της ιδιότητας.
### getPropertyInsets(String key) {#getPropertyInsets-java.lang.String-}
```
public Insets getPropertyInsets(String key)
```


Λαμβάνει την τιμή της ιδιότητας insets.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.awt.Insets - Τιμή της ιδιότητας.
### getPropertyInsets(String key, Insets def) {#getPropertyInsets-java.lang.String-java.awt.Insets-}
```
public Insets getPropertyInsets(String key, Insets def)
```


Λαμβάνει την τιμή της ιδιότητας insets. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.awt.Insets | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.awt.Insets - Τιμή της ιδιότητας.
### getPropertyInt(String key) {#getPropertyInt-java.lang.String-}
```
public int getPropertyInt(String key)
```


Λαμβάνει την τιμή της ιδιότητας integer.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
int - Τιμή της ιδιότητας.
### getPropertyInt(String key, int def) {#getPropertyInt-java.lang.String-int-}
```
public int getPropertyInt(String key, int def)
```


Λαμβάνει την τιμή της ιδιότητας ακέραιου. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | int | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
int - Τιμή της ιδιότητας.
### getPropertyMatrix(String key) {#getPropertyMatrix-java.lang.String-}
```
public AffineTransform getPropertyMatrix(String key)
```


Λαμβάνει την τιμή της ιδιότητας float.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.awt.geom.AffineTransform - Τιμή της ιδιότητας.
### getPropertyMatrix(String key, AffineTransform def) {#getPropertyMatrix-java.lang.String-java.awt.geom.AffineTransform-}
```
public AffineTransform getPropertyMatrix(String key, AffineTransform def)
```


Λαμβάνει την τιμή της ιδιότητας float. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.awt.geom.AffineTransform | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.awt.geom.AffineTransform - Τιμή της ιδιότητας.
### getPropertyRectangle(String key) {#getPropertyRectangle-java.lang.String-}
```
public Rectangle getPropertyRectangle(String key)
```


Λαμβάνει την τιμή της ιδιότητας rectangle.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.awt.Rectangle - Τιμή της ιδιότητας.
### getPropertyRectangle(String key, Rectangle def) {#getPropertyRectangle-java.lang.String-java.awt.Rectangle-}
```
public Rectangle getPropertyRectangle(String key, Rectangle def)
```


Λαμβάνει την τιμή της ιδιότητας rectangle. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την παρεχόμενη προεπιλεγμένη τιμή.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.awt.Rectangle | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.awt.Rectangle - Τιμή της ιδιότητας.
### getPropertyStringArray(String key) {#getPropertyStringArray-java.lang.String-}
```
public String[] getPropertyStringArray(String key)
```


Λαμβάνει την τιμή της ιδιότητας string array.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
java.lang.String[] - Τιμή ιδιότητας.
### getPropertyStringArray(String key, String[] def) {#getPropertyStringArray-java.lang.String-java.lang.String---}
```
public String[] getPropertyStringArray(String key, String[] def)
```


Λαμβάνει την τιμή ιδιότητας πίνακα συμβολοσειρών. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την προεπιλεγμένη τιμή που παρέχεται.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | java.lang.String[] | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
java.lang.String[] - Τιμή ιδιότητας.
### hashCode() {#hashCode--}
```
public synchronized int hashCode()
```




**Returns:**
int
### isEmpty() {#isEmpty--}
```
public boolean isEmpty()
```




**Returns:**
boolean
### isProperty(String key) {#isProperty-java.lang.String-}
```
public boolean isProperty(String key)
```


Λαμβάνει την τιμή της ιδιότητας boolean.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |

**Returns:**
boolean - Τιμή ιδιότητας.
### isProperty(String key, boolean def) {#isProperty-java.lang.String-boolean-}
```
public boolean isProperty(String key, boolean def)
```


Λαμβάνει την τιμή ιδιότητας boolean. Εάν η ζητούμενη ιδιότητα λείπει, επιστρέφει την προεπιλεγμένη τιμή που παρέχεται.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| def | boolean | Προεπιλεγμένη τιμή της ιδιότητας. |

**Returns:**
boolean - Τιμή ιδιότητας.
### keySet() {#keySet--}
```
public Set<Object> keySet()
```




**Returns:**
java.util.Set<java.lang.Object>
### keys() {#keys--}
```
public Enumeration<Object> keys()
```




**Returns:**
java.util.Enumeration<java.lang.Object>
### list(PrintStream arg0) {#list-java.io.PrintStream-}
```
public void list(PrintStream arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.PrintStream |  |

### list(PrintWriter arg0) {#list-java.io.PrintWriter-}
```
public void list(PrintWriter arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.PrintWriter |  |

### load(InputStream arg0) {#load-java.io.InputStream-}
```
public synchronized void load(InputStream arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.InputStream |  |

### load(Reader arg0) {#load-java.io.Reader-}
```
public synchronized void load(Reader arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.Reader |  |

### loadFromXML(InputStream arg0) {#loadFromXML-java.io.InputStream-}
```
public synchronized void loadFromXML(InputStream arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.InputStream |  |

### merge(K arg0, V arg1, BiFunction<? super V,? super V,? extends V> arg2) {#merge-K-V-java.util.function.BiFunction---super-V---super-V---extends-V--}
```
public synchronized V merge(K arg0, V arg1, BiFunction<? super V,? super V,? extends V> arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | V |  |
| arg2 | java.util.function.BiFunction<? super V,? super V,? extends V> |  |

**Returns:**
V
### merge(Object arg0, Object arg1, BiFunction<? super Object,? super Object,?> arg2) {#merge-java.lang.Object-java.lang.Object-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----}
```
public synchronized Object merge(Object arg0, Object arg1, BiFunction<? super Object,? super Object,?> arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |
| arg2 | java.util.function.BiFunction<? super java.lang.Object,? super java.lang.Object,?> |  |

**Returns:**
java.lang.Object
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### printProperties() {#printProperties--}
```
public void printProperties()
```


Εκτυπώνει όλες τις ορισμένες ιδιότητες.

### propertyNames() {#propertyNames--}
```
public Enumeration propertyNames()
```


Επιστρέφει τα ονόματα των ιδιοτήτων.

**Returns:**
java.util.Enumeration - Απαρίθμηση ονομάτων ιδιοτήτων.
### put(K arg0, V arg1) {#put-K-V-}
```
public synchronized V put(K arg0, V arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | V |  |

**Returns:**
V
### put(Object arg0, Object arg1) {#put-java.lang.Object-java.lang.Object-}
```
public synchronized Object put(Object arg0, Object arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |

**Returns:**
java.lang.Object
### putAll(Map<? extends K,? extends V> arg0) {#putAll-java.util.Map---extends-K---extends-V--}
```
public synchronized void putAll(Map<? extends K,? extends V> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.Map<? extends K,? extends V> |  |

### putAll(Map<?,?> arg0) {#putAll-java.util.Map------}
```
public synchronized void putAll(Map<?,?> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.Map<?,?> |  |

### putIfAbsent(K arg0, V arg1) {#putIfAbsent-K-V-}
```
public synchronized V putIfAbsent(K arg0, V arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | V |  |

**Returns:**
V
### putIfAbsent(Object arg0, Object arg1) {#putIfAbsent-java.lang.Object-java.lang.Object-}
```
public synchronized Object putIfAbsent(Object arg0, Object arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |

**Returns:**
java.lang.Object
### remove(Object arg0) {#remove-java.lang.Object-}
```
public synchronized Object remove(Object arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
java.lang.Object
### remove(Object arg0, Object arg1) {#remove-java.lang.Object-java.lang.Object-}
```
public synchronized boolean remove(Object arg0, Object arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |

**Returns:**
boolean
### replace(K arg0, V arg1) {#replace-K-V-}
```
public synchronized V replace(K arg0, V arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | V |  |

**Returns:**
V
### replace(K arg0, V arg1, V arg2) {#replace-K-V-V-}
```
public synchronized boolean replace(K arg0, V arg1, V arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | K |  |
| arg1 | V |  |
| arg2 | V |  |

**Returns:**
boolean
### replace(Object arg0, Object arg1) {#replace-java.lang.Object-java.lang.Object-}
```
public synchronized Object replace(Object arg0, Object arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |

**Returns:**
java.lang.Object
### replace(Object arg0, Object arg1, Object arg2) {#replace-java.lang.Object-java.lang.Object-java.lang.Object-}
```
public synchronized boolean replace(Object arg0, Object arg1, Object arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.lang.Object |  |
| arg1 | java.lang.Object |  |
| arg2 | java.lang.Object |  |

**Returns:**
boolean
### replaceAll(BiFunction<? super K,? super V,? extends V> arg0) {#replaceAll-java.util.function.BiFunction---super-K---super-V---extends-V--}
```
public synchronized void replaceAll(BiFunction<? super K,? super V,? extends V> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.function.BiFunction<? super K,? super V,? extends V> |  |

### replaceAll(BiFunction<? super Object,? super Object,?> arg0) {#replaceAll-java.util.function.BiFunction---super-java.lang.Object---super-java.lang.Object----}
```
public synchronized void replaceAll(BiFunction<? super Object,? super Object,?> arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.util.function.BiFunction<? super java.lang.Object,? super java.lang.Object,?> |  |

### save(OutputStream arg0, String arg1) {#save-java.io.OutputStream-java.lang.String-}
```
public void save(OutputStream arg0, String arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.OutputStream |  |
| arg1 | java.lang.String |  |

### setProperties(Properties properties) {#setProperties-java.util.Properties-}
```
public void setProperties(Properties properties)
```


Αντιγράφει τις ιδιότητες, συμπεριλαμβανομένων των προεπιλογών τους, σε αυτό το UserProperties

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ιδιότητες. |

### setProperty(String key, boolean value) {#setProperty-java.lang.String-boolean-}
```
public Object setProperty(String key, boolean value)
```


Ορίζει την τιμή της ιδιότητας boolean.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | boolean | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, double value) {#setProperty-java.lang.String-double-}
```
public Object setProperty(String key, double value)
```


Ορίζει την τιμή της ιδιότητας double.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | double | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, float value) {#setProperty-java.lang.String-float-}
```
public Object setProperty(String key, float value)
```


Ορίζει την τιμή της ιδιότητας float.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | float | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, int value) {#setProperty-java.lang.String-int-}
```
public Object setProperty(String key, int value)
```


Ορίζει την τιμή της ιδιότητας integer.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | int | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, Color value) {#setProperty-java.lang.String-java.awt.Color-}
```
public Object setProperty(String key, Color value)
```


Ορίζει την τιμή της ιδιότητας χρώματος.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Color | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, Dimension value) {#setProperty-java.lang.String-java.awt.Dimension-}
```
public Object setProperty(String key, Dimension value)
```


Ορίζει την τιμή της ιδιότητας διαστάσεων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Dimension | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, Insets value) {#setProperty-java.lang.String-java.awt.Insets-}
```
public Object setProperty(String key, Insets value)
```


Ορίζει την τιμή της ιδιότητας insets.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Insets | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, Rectangle value) {#setProperty-java.lang.String-java.awt.Rectangle-}
```
public Object setProperty(String key, Rectangle value)
```


Ορίζει την τιμή της ιδιότητας rectangle.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Rectangle | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, AffineTransform value) {#setProperty-java.lang.String-java.awt.geom.AffineTransform-}
```
public Object setProperty(String key, AffineTransform value)
```


Ορίζει την τιμή της ιδιότητας matrix.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.geom.AffineTransform | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, String value) {#setProperty-java.lang.String-java.lang.String-}
```
public Object setProperty(String key, String value)
```


Ορίζει την τιμή της ιδιότητας string.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.lang.String | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(String key, String[] value) {#setProperty-java.lang.String-java.lang.String---}
```
public Object setProperty(String key, String[] value)
```


Ορίζει την τιμή της ιδιότητας string array.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.lang.String[] | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, boolean value) {#setProperty-java.util.Properties-java.lang.String-boolean-}
```
public static Object setProperty(Properties properties, String key, boolean value)
```


Ορίζει την τιμή της ιδιότητας boolean σε καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | boolean | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, double value) {#setProperty-java.util.Properties-java.lang.String-double-}
```
public static Object setProperty(Properties properties, String key, double value)
```


Ορίζει την τιμή της ιδιότητας double σε καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | double | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, float value) {#setProperty-java.util.Properties-java.lang.String-float-}
```
public static Object setProperty(Properties properties, String key, float value)
```


Ορίζει την τιμή της ιδιότητας float στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | float | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, int value) {#setProperty-java.util.Properties-java.lang.String-int-}
```
public static Object setProperty(Properties properties, String key, int value)
```


Ορίζει την τιμή της ιδιότητας integer στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | int | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, Color value) {#setProperty-java.util.Properties-java.lang.String-java.awt.Color-}
```
public static Object setProperty(Properties properties, String key, Color value)
```


Ορίζει την τιμή της ιδιότητας color στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Color | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, Dimension value) {#setProperty-java.util.Properties-java.lang.String-java.awt.Dimension-}
```
public static Object setProperty(Properties properties, String key, Dimension value)
```


Ορίζει την τιμή της ιδιότητας dimension στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Dimension | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, Insets value) {#setProperty-java.util.Properties-java.lang.String-java.awt.Insets-}
```
public static Object setProperty(Properties properties, String key, Insets value)
```


Ορίζει την τιμή της ιδιότητας insets στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Insets | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, Rectangle value) {#setProperty-java.util.Properties-java.lang.String-java.awt.Rectangle-}
```
public static Object setProperty(Properties properties, String key, Rectangle value)
```


Ορίζει την τιμή της ιδιότητας rectangle στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.Rectangle | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, AffineTransform value) {#setProperty-java.util.Properties-java.lang.String-java.awt.geom.AffineTransform-}
```
public static Object setProperty(Properties properties, String key, AffineTransform value)
```


Ορίζει την τιμή της ιδιότητας matrix στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.awt.geom.AffineTransform | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### setProperty(Properties properties, String key, String[] value) {#setProperty-java.util.Properties-java.lang.String-java.lang.String---}
```
public static Object setProperty(Properties properties, String key, String[] value)
```


Ορίζει την τιμή της ιδιότητας string array στον καθορισμένο πίνακα ιδιοτήτων.

**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| ιδιότητες | java.util.Properties | Ο πίνακας ιδιοτήτων. |
| κλειδί | java.lang.String | Το όνομα της ιδιότητας. |
| τιμή | java.lang.String[] | Η τιμή της ιδιότητας. |

**Returns:**
java.lang.Object - Μια ιδιότητα.
### size() {#size--}
```
public int size()
```




**Returns:**
int
### store(OutputStream arg0, String arg1) {#store-java.io.OutputStream-java.lang.String-}
```
public void store(OutputStream arg0, String arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.OutputStream |  |
| arg1 | java.lang.String |  |

### store(Writer arg0, String arg1) {#store-java.io.Writer-java.lang.String-}
```
public void store(Writer arg0, String arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.Writer |  |
| arg1 | java.lang.String |  |

### storeToXML(OutputStream arg0, String arg1) {#storeToXML-java.io.OutputStream-java.lang.String-}
```
public void storeToXML(OutputStream arg0, String arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.OutputStream |  |
| arg1 | java.lang.String |  |

### storeToXML(OutputStream arg0, String arg1, String arg2) {#storeToXML-java.io.OutputStream-java.lang.String-java.lang.String-}
```
public void storeToXML(OutputStream arg0, String arg1, String arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.OutputStream |  |
| arg1 | java.lang.String |  |
| arg2 | java.lang.String |  |

### storeToXML(OutputStream arg0, String arg1, Charset arg2) {#storeToXML-java.io.OutputStream-java.lang.String-java.nio.charset.Charset-}
```
public void storeToXML(OutputStream arg0, String arg1, Charset arg2)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | java.io.OutputStream |  |
| arg1 | java.lang.String |  |
| arg2 | java.nio.charset.Charset |  |

### stringPropertyNames() {#stringPropertyNames--}
```
public Set<String> stringPropertyNames()
```




**Returns:**
java.util.Set<java.lang.String>
### toString() {#toString--}
```
public synchronized String toString()
```




**Returns:**
java.lang.String
### values() {#values--}
```
public Collection<Object> values()
```




**Returns:**
java.util.Collection<java.lang.Object>
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| Παράμετρος | Τύπος | Περιγραφή |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

