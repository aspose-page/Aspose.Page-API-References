---
title: Class DocumentURI
second_title: Aspose.Page για Αναφορά API .NET
description: Aspose.Page.XPS.XpsMetadata.DocumentURI τάξη. Καθορίζει ένα ενιαίο αναγνωριστικό πόρου URI για το έγγραφο. https//docs.microsoft.com/enus/windows/win32/printdocs/documenturi
type: docs
weight: 840
url: /el/net/aspose.page.xps.xpsmetadata/documenturi/
---
## DocumentURI class

Καθορίζει ένα ενιαίο αναγνωριστικό πόρου (URI) για το έγγραφο. https://docs.microsoft.com/en-us/windows/win32/printdocs/documenturi

```csharp
public sealed class DocumentURI : URIProperty
```

## Κατασκευαστές

| Ονομα | Περιγραφή |
| --- | --- |
| [DocumentURI](documenturi/)(string) | Δημιουργεί μια νέα παρουσία. |

## Ιδιότητες

| Ονομα | Περιγραφή |
| --- | --- |
| [Name](../../aspose.page.xps.xpsmetadata/printticketelement/name/) { get; } | Παίρνει το όνομα του στοιχείου. |

### Δείτε επίσης

* class [URIProperty](../uriproperty/)
* χώρος ονομάτων [Aspose.Page.XPS.XpsMetadata](../../aspose.page.xps.xpsmetadata/)
* συνέλευση [Aspose.Page](../../)


